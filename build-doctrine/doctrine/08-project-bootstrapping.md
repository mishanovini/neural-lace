...
- **Failure-Mode catalog (cross-cutting, every project)** — every bootstrapped project adopts a single `docs/failure-modes.md` (FM = Failure Mode) per the cross-project standard `docs/conventions/failure-mode-catalogs.md` (Decision 033). Bootstrap step: copy `docs/templates/project-failure-modes/failure-modes.md` into the new project's `docs/`, add the investigation-first one-liner to its `CLAUDE.md`/`AGENTS.md`, commit. Thereafter the harness's global `rules/diagnosis.md` makes catalog-grep the first step of any investigation and catalog-extend the last step of any fix — uniform across every project without per-project hook wiring. This is a universal floor in spirit (a documentation-and-diagnosis convention) layered on top of Floor #8; the per-project file is mandatory, the optional Discriminator/Recovery fields are populated when they add signal.
