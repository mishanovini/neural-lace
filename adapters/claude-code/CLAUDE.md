...
- **Drive to completion. Do not end the turn between sub-tasks if there is more work to do.** A code session ends its turn only when (a) all assigned work is complete and pushed, (b) you hit a genuine blocker requiring user input (Tier 3 per `planning.md` — irreversible op, ambiguous product decision, missing credentials), or (c) the user has explicitly said "stop" / "pause" / "that's enough" in their most-recent message. Stopping for any other reason — "natural breakpoint," "good place to check in," "ready for the next phase" — is narrate-and-wait behavior and is prohibited when autonomous execution was authorized. **Every turn — completed, paused, or blocked — MUST end with EXACTLY ONE marker alone on the last non-empty line of the final response: `DONE: <what shipped>`, `PAUSING: <the user decision needed + the exact question>`, or `BLOCKED: <the specific blocker + what unblocks it>`.** This is the structured, machine-checked form of the old `WHY I STOPPED:` convention; the `continuation-enforcer.sh` Stop hook blocks session-end without a valid marker. If there is more declared work and no blocker, do NOT add a marker — keep working (there is no marker for a no-reason pause). Cross-reference: `~/.claude/rules/session-end-protocol.md`, `~/.claude/rules/testing.md` "Keep Going When Keep-Going Is Authorized", `narrate-and-wait-gate.sh` + `continuation-enforcer.sh` Stop hooks.
...
- `diagnosis.md` — exhaustive diagnosis before fixing, full-chain tracing, the **DIAGNOSTIC-FIRST PROTOCOL** (pull runtime/error logs as the FIRST tool call on any production-failure investigation — `vercel logs` / Sentry / Datadog / Supabase logs / webhook delivery logs / job-runner logs — inferential evidence permitted only after logs are examined or an explicit inaccessibility acknowledgment), the investigation-first Failure-Mode-catalog reflex (grep `docs/failure-modes.md` before forming a hypothesis), "Fix the Class, Not the Instance" sub-rule. Cross-project FM-catalog standard: `docs/conventions/failure-mode-catalogs.md` (Decision 033). Originating case study: `docs/lessons/2026-05-22-fm-001-misdiagnosis.md`.
- `claims.md` — hypothesis-vs-proof labeling (every causal claim tagged PROVEN with cited evidence OR HYPOTHESIZED with refutation criterion; naked confident phrasing prohibited) + refutation-criteria requirement (before authoring an implementation plan on top of a hypothesis, explicitly write the refutation criterion AND look for refuting evidence before committing engineering resources). Pairs with `diagnosis.md`'s diagnostic-first protocol. Decision 035.
