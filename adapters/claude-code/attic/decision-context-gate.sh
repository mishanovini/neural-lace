...
#
# RELOCATION NOTE (NL Overhaul Wave D, task D.4, per specs-d.md §D.4 item 4
# + §D.0.2's retired-from-Stop list): per the frozen Wave-D disposition,
# this gate's BLOCKING fence-ENFORCEMENT behavior retires to attic/ at D.5
# — the emit-side writers (decision-context-reply-emit.sh), the pending-
# decision ledger, and decision-context-pending-surfacer.sh all STAY (they
# are not blocking and are not touched by this relocation). This file's own
# behavior is UNCHANGED by this note; D.5 is the cutover that unwires it
# from the live Stop chain (moves this file to attic/ + a live-path exit-0
# shim). No code below this comment has been modified for D.4.
#
