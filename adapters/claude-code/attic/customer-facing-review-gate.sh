...
#
# RELOCATION NOTE (NL Overhaul Wave D, task D.4, per specs-d.md §D.4 item 2):
# the classifier pattern list (STRONG_RE / WEAK_RE / EXCLUSION_RE below) has
# been extracted verbatim to patterns/customer-facing-patterns.txt, and
# hooks/teammate-spawn-validator.sh now sources it to emit a NON-BLOCKING
# spawn-time warn (+ signal-ledger event) when a spawned task prompt matches
# a customer-facing pattern. This file's own BLOCKING Stop-hook enforcement
# is UNCHANGED here — D.5 is the cutover that unwires this gate from the
# live Stop chain (moves it to attic/ + a live-path exit-0 shim). Until D.5
# lands, both this gate's block-mode enforcement AND the new spawn-time warn
# are active in parallel (the warn does not replace the block early).
#
