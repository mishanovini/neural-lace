...
#
# folded into teammate-spawn-validator (Wave D); unwired at D.5.
#
# NL Overhaul Wave D.6 (§D.6 item 2, 2026-07-02): this gate's check was
# folded into adapters/claude-code/hooks/teammate-spawn-validator.sh as
# its rejection condition (d) — see `_dag_inspect_dispatch` and the
# `_dag_*` helper functions there, which are a faithful port of the
# logic below. teammate-spawn-validator.sh is now the ONE gate emitting
# ONE combined block message for both Agent-Teams spawn safety AND
# DAG-review waiver enforcement. THIS FILE stays in place (unchanged
# below) so any stale live PreToolUse wiring pointing directly at it
# keeps working during the release window; the actual PreToolUse
# registration for this file retires (template unwiring) at D.5, after
# which this file moves to attic/.
#
