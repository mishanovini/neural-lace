...
The companion `completion-criteria-gate` (in-flight 2026-06-01) carries an N-criterion session-completion checklist. UX/CX review is added as a criterion for customer-facing work (`UX agent + customer-advocate agent review attached`, N/A for backend-only). Because that gate lives uncommitted in a sibling worktree, the criterion spec is handed off via `.claude/state/spawned-task-results/` (see ADR 046) rather than edited cross-worktree.
...
- `docs/decisions/046-customer-facing-review-gate.md` — the ADR (failure-mode cost made explicit).
