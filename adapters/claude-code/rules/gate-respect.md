...
Worked example: HARNESS-GAP-27 (2026-05-14) tracks the `scope-enforcement-gate.sh` blind-spot on merge commits. The gate iterates currently-ACTIVE plans, but a merge of master pulls in files from plans that were ACTIVE on master and have since archived. The lightweight fix is to extend the gate's `_is_system_managed_path()` allowlist with migration-class paths when `$GIT_DIR/MERGE_HEAD` exists; the more-general fix (check against UNION of plans active on either side of the merge) needs an ADR. Both paths are documented in the entry. The bypass-per-occurrence workaround is documented too, but the goal is to make it unnecessary. *(Update 2026-05-27: the blind-spot was closed by PR #26 / HARNESS-GAP-29 — the gate now full-skips during any rebase/merge-in-progress, superseding the migration-only allowlist; HARNESS-GAP-27 and ADR 030's union-of-plans design are marked superseded. The lesson — fix the gate's recurring false-positive rather than bypass per-occurrence — is exactly what landed.)*
