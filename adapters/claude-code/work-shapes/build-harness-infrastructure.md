...
| `plan-reviewer.sh` Check 4b (walking-skeleton integration-vaporware defense) | Blocks plans that lack a `## Walking Skeleton` section identifying the thinnest end-to-end slice touching every architectural layer (UI → API → worker → DB → notification) | **Advisory (warn-only)** — the check still runs and findings are surfaced on stderr, but the gate does not block when every file in `## Files to Modify/Create` resolves to a path under `adapters/claude-code/` or `~/.claude/`. Rationale: harness mechanisms have no user-observable runtime; "the user clicks X and sees Y" doesn't apply when the "user" is a hook firing at an event boundary and the observable behavior is `--self-test` passing. |
| `plan-reviewer.sh` Check 13 (integration verification for `Verification: full` tasks) | Blocks plans whose `Verification: full` tasks lack the three required sub-blocks (`**Prove it works:**`, `**Wire checks:**`, `**Integration points:**`) | **Advisory (warn-only)** — same harness-internal scope as Check 4b. The integration-verification contract (browser-replayable scenario, UI→API→DB code chain in backtick arrows) is incoherent for harness mechanisms whose layer-count is one. Typical harness plans declare `Verification: mechanical` on every task and never trigger Check 13 anyway; the carve-out is a belt-and-suspenders defense for the rare case where a harness task is `Verification: full`. |
...
- **Treating Check 4b / Check 13 advisory as "ignore it."** Both checks still run and surface findings on stderr. Read the findings — they may name a real integration gap. Advisory means "this won't block the commit," not "this won't tell you anything useful." If the finding is genuinely irrelevant (no user surface exists), proceed; if it points at a real ambiguity in how the mechanism integrates, fix it before commit. Simplest paths: declare `Walking Skeleton: n/a — harness-internal work; self-test is the end-to-end slice` on a single line in the plan to satisfy Check 4b substantively, AND declare `Verification: mechanical` on harness tasks to make Check 13 a no-op rather than relying on the advisory path.
...
## Notes on advisory-mode Check 4b and Check 13

`plan-reviewer.sh` Check 4b (walking-skeleton integration-vaporware defense) and Check 13 (integration verification for `Verification: full` tasks) are BOTH advisory when EVERY entry in the plan's `## Files to Modify/Create` section resolves to a path under `adapters/claude-code/` or `~/.claude/`. The same harness-internal scope detector (`IS_HARNESS_INTERNAL`) gates both checks. Specifically:

- Each check still parses the plan and looks for its respective contract (Check 4b: the `## Walking Skeleton` section; Check 13: per-task `**Prove it works:**` / `**Wire checks:**` / `**Integration points:**` sub-blocks on `Verification: full` tasks).
- Findings are still printed to stderr — so the maintainer sees what each check would have surfaced.
- The findings are NOT added to `FINDING_COUNT` (the gate's blocking aggregate), so the plan can move to ACTIVE without authoring a walking-skeleton section OR the integration sub-blocks.
- A single product-file path in `## Files to Modify/Create` flips the plan back to blocking-mode for BOTH checks. Mixed-scope plans (product + harness) are treated as product-scope.
...
- Check 13 is independently exempt for tasks declaring `Verification: mechanical` or `Verification: contract` (per Tranche D of architecture-simplification). Typical harness plans declare `mechanical` on every task and never trigger Check 13 regardless of the harness-internal carve-out — the carve-out is a belt-and-suspenders defense for the rare case where a harness task is `Verification: full`.

Rationale: harness mechanisms have no user-observable runtime. "The user clicks the Save button and sees the form persist" doesn't apply when the "user" is a `PreToolUse` hook firing at an event boundary and the "observable behavior" is `--self-test` passing. The contract Check 4b enforces (thinnest end-to-end slice through every architectural layer) and the contract Check 13 enforces (browser-replayable scenario + UI→API→DB code chain in backtick arrows) are both incoherent for harness infrastructure: the layer-count is one (the hook itself) and the slice IS the self-test. Keeping both checks advisory preserves the audit trail without imposing a Goodhart-shaped friction on every harness commit.
