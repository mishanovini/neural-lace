...
- Design-projected plans carry `design-ref: <path>@<blob>` + `## Review Chain`; per-task `Implements:`/`Directives:` when design-ref is real (Checks 20-22 enforce; full field spec, triggers, escape, dates: doctrine/planning-full.md).
...
- Mid-build decisions: two-tier by reversibility (constitution §8) — reversible → decide-and-go (log, proceed, batch-present); hard-to-reverse per the §8 list → pause with options + recommendation.
