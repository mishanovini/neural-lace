...
**Gates that fire identically regardless of team mode** (no team-awareness needed): `pre-commit-tdd-gate.sh`, `harness-hygiene-scan.sh`, `pre-push-scan.sh`, `scope-enforcement-gate.sh` (PreToolUse Bash on `git commit` — fires per teammate's commit attempt; if the staged files fall outside the active plan's `## Files to Modify/Create` or `## In-flight scope updates` sections the gate blocks the same way it does in solo mode), `dag-review-waiver-gate.sh` (PreToolUse on the `Task`/`Agent` tool — gates the lead's first teammate-spawn per session when a Tier 3+ plan is active; teammates cannot spawn teammates per Anthropic's no-nested-subagents constraint, so this gate never fires from teammate context), and inline PreToolUse Bash blockers (force-push, `--no-verify`, public-repo, dangerous-command pattern). These run at git operations or per-session against repo state.
