# Operator-requirement ledger — full

This is the detail companion to `operator-requirement-ledger.md` (the compact, capped at
3000 bytes by `evals/golden/rules-index-coverage.sh`). It holds the full step-by-step
discipline detail trimmed out of the compact on 2026-08-03 to bring it back under the cap —
the compact's summary and honest-limits section stand alone; this file is the full mechanism.
...
## The discipline — full step-by-step
...
1. **Store the sentence verbatim.** Never a paraphrase — the paraphrase IS the
   ...
   Use the id it prints — steps 2–3 refuse a `--requirement-id` no requirement
   matches (such a record is unreachable from both selectors, so a clause
   vanishes while `invariant-check` says "nothing registered"). Over
   ~4000 chars the store appends `[TRUNCATED: N chars omitted]`, never a silent
   clip. `--verbatim-ref` is rejected here — it would overwrite the ask's
   pointer to the original prompt; set it on `register`.
2. **Decompose it into separately checkable invariants.** Ask of every clause:
   *what would still be true if I built this right?* Each answer is one
   ...
   No `--ask-id` — it is resolved from the requirement.
3. **Return a verdict per invariant, with a citation.**
   `invariant-verdict --requirement-id <r> --invariant-id <i> --verdict
   holds|violated|unverifiable --evidence <file:line|path|command|SHA>`.
   An unevidenced `holds` is the defect itself, so it is refused — as is one
   whose evidence is blank-after-trim or could not be a citation in any reading
   (`x`, `.`, `-`, `trust me`). That test is purely SYNTACTIC (`:`+digit, 7+
   hex, or `/`) and judges nothing about quality. `violated`/`unverifiable`
   need no evidence.
4. **Check before claiming done.** `invariant-check --plan-slug <slug>`
   ...
   evaluate** (unreadable store — e.g. one torn JSONL line). 3 and 4 are NOT
   passes; a degraded read is never green.

Storage is `ask-registry.sh` — no new store, per the ≤3-store consolidation
goal. `invariants` prints the TSV. Verdicts fold last-wins; absence folds to
