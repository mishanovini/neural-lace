...
- Build in parallel, verify sequentially: builders build+commit in worktrees, return verdict+worktree_path+commit_shas+summary WITHOUT invoking task-verifier; orchestrator cherry-picks results in task-ID order, invokes task-verifier, tears down the worktree. Conflict = wrong parallelism: abort, mark BLOCKED, never force-resolve.
...
- Dispatch prompts are self-contained: plan path, task IDs, branch+HEAD, env vars, scenarios verbatim — NEVER the assertion list (scenarios-shared, assertions-private). MUST open with `NL-ATTRIBUTION: plan=<slug> task=<id> role=<builder|verifier|reviewer|advocate>`; WARN-only; quoted-header residual: prefix `> `. **Generate, never hand-type**: `bash adapters/claude-code/scripts/dispatch-directives.sh <plan> <task-N> [role]` prints the header + `Directives:` block, verbatim — detail: -full.md.
...
- **Proactive audit loop:** before declaring a product area done, self-drive a static failed-functionality hunt + runtime exercise of every claimed flow, fix, re-run, repeat to a clean pass. Bar: surface problems the operator didn't point at. Six failure patterns + origin incident: -full.md.
