...
- Shared-checkout git-state disciplines (incidents 2026-07-06/07/09; full detail + incident record in -full.md): WORKTREE-CHECK line in every brief (builder confirms `git rev-parse --show-toplevel` is its OWN worktree); BRANCH-VERIFY before every shared-checkout commit + `ls-remote` to prove a push; COMMIT-VERIFY-AFTER-DENIAL (a denied compound `add && commit && push` kills the commit too — `git log -1` before assuming it exists).
