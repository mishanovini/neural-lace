...
- Shared-checkout git-state disciplines (mechanics + incidents in -full.md): WORKTREE-CHECK (builder confirms its OWN worktree via `git rev-parse --show-toplevel`); BRANCH-VERIFY + `ls-remote` around every shared-checkout commit/push; COMMIT-VERIFY-AFTER-DENIAL (a denied `add && commit && push` kills the commit — `git log -1` first).
...
- **Proactive audit loop** before declaring a product area done (incident 2026-06-18; the 6 dead-code patterns + full protocol are in -full.md): (a) static failed-functionality hunt; (b) runtime-exercise EVERY claimed flow and verify the outcome matches the claim; (c) fix → re-audit → repeat to a clean pass. Bar: surface problems the operator did NOT point at, or the audit failed.
