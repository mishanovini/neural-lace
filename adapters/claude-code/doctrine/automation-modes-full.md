...
|---|---|---|---|---|
| **Background-task heartbeat** | NOT running (woken only when it fires) | External scheduled task; output is filesystem state | Sweeping state the agent observes on next session start | `workstreams-emit.sh --heartbeat` (designed Layer-C backstop — NOT currently registered with any scheduler; wire-or-delete tracked as NL-FINDING-022); `session-resilience` PostToolUse handoff write |
