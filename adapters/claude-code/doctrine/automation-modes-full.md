...
|---|---|---|---|---|
| **Background-task heartbeat** | NOT running (woken only when it fires) | External scheduled task; output is filesystem state | Sweeping state the agent observes on next session start | `workstreams-emit.sh --heartbeat` (Layer-C backstop; registered as the Windows scheduled task `NL-workstreams-heartbeat` by install.sh, every 5 min — NL-FINDING-022 WIRE decision, specs-e §E.10 item 6; doctor predicate verifies task existence); `session-resilience` PostToolUse handoff write |
