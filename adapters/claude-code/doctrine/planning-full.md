...
**Configurable controls (permission toggles, feature flags, settings, dropdowns) — the decorative-control corollary.** A configurable control is done only when changing it is PROVEN to change observable behavior at the surface it claims to govern. Rendering is not done, persisting is not done, and wired-to-shadow-mode is not done — a control that renders, saves, and changes nothing is the decorative-config-control vaporware class (FM-038 in `docs/failure-modes.md`). When demonstrating, exercise the control at two or more values the spec claims produce DIFFERENT behavior — two test values that legitimately fall in the same behavior bucket (e.g., both sides of nothing on a threshold) prove nothing. Shadow-mode (log-only) wiring is a legitimate rollout state only while it is declared and time-bounded with an owned flip-to-enforce obligation. The registry-vs-callsite invariant and project-level drift-check recipe live in `doctrine/vaporware-prevention-full.md`.
...
**Two writing styles are accepted.** The validator recognizes either:

- **Strict scaffold form** — fill in the `### a)` / `### b)` / `### c)` sub-heading the template ships with; replace the bracketed placeholder under your chosen sub-heading with the answer; leave the other two sub-headings present with their placeholders intact (they document the option set).
- **AI-natural prose form** — replace the three `###` scaffolds with a paragraph that starts with `(a)`, `(b)`, or `(c)` (optionally bold-wrapped as `**(b) New catalog entry proposed.**`) followed by the substantive answer. This is the form AI-spawned PRs typically produce; the validator falls back to it when the strict scaffold form isn't detected. For (c), the ≥40-char substantive rationale requirement still applies to the prose body.

**The template lives at** `.github/PULL_REQUEST_TEMPLATE.md` in the neural-lace repo (and downstream repos that have run the rollout script). When you open a PR via `gh pr create` or the GitHub UI, the body is auto-populated with the four required sections (Summary, What changed and why, **What mechanism would have caught this?**, Testing performed). Use either writing style above.
