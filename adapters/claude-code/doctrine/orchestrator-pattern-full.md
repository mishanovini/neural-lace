...
## NL-ATTRIBUTION header — machine-readable dispatch attribution (2026-07-29)

**Why this exists.** Operator directive 2026-07-29: "how do we ensure we don't keep running into this same damn issue of you reporting something that's complete false?" Measured root cause: `workstreams-emit.sh`'s `--on-builder-dispatch` PreToolUse hook already fires reliably (1000+ real `source=emit-feed` rows/day in `~/.claude/state/governor/ledger/`), but nothing in a dispatch is MACHINE-READABLE — the pre-existing free-text heuristic (`_extract_plan_slug`/`_extract_task_id`) only catches prompts that literally contain `docs/plans/<slug>.md` and `Task N of` phrasing, so most dispatches (including this convention's own rollout dispatch — see below) silently no-op, and `roadmap-routes.js`'s `deriveLiveAgentLeaves` binds nothing. The operator sees "N running, unattributed to a task" while the session reports more agents in flight, and no surface can arbitrate.

**The format.** Every dispatch prompt (`Task`/`Agent`/`Workflow` tool calls, and `mcp__ccd_session__spawn_task`) MUST **OPEN WITH** this line — it must start a line, and sit within the first `NL_ATTRIBUTION_MAX_LINE` (default 5) lines of the JOINED `prompt + description + content` (see the residual note below — the window is NOT over the prompt alone):

    NL-ATTRIBUTION: plan=<slug> task=<id> role=<builder|verifier|reviewer|advocate>

- `plan=<slug>` — the plan file slug (`docs/plans/<slug>.md`, no path/ext).
- `task=<id>` — the task id EXACTLY as written in the plan (`3`, `3.2`, `T4` — a free token, not numeric-only, since accountable-estate-style plans use `T`-prefixed ids).
- `role=<...>` — closed enum (`builder|verifier|reviewer|advocate`); an unrecognized value is dropped, never guessed.

**WHY THE POSITION IS PART OF THE FORMAT (corrected 2026-07-30 — this section previously said "a line anywhere in the prompt text", and that looser wording WAS the bug's charter).** A header matched anywhere is indistinguishable from a header being QUOTED: a prompt merely DISCUSSING a prior dispatch ("…it was dispatched with the line `NL-ATTRIBUTION: plan=X task=9 role=builder` and it failed") emitted a real `task_started` and turned task 9's chip green. That is the same mention-is-not-a-dispatch defect the scrape removal was for, surviving inside the source declared authoritative — and it is a live vector, because handoff, review and post-mortem prompts routinely paste the builder prompt they are about. So the parser now enforces what this doctrine already said in words elsewhere ("MUST open with"): line-start, within the first `NL_ATTRIBUTION_MAX_LINE` (default 5) lines of the JOINED `prompt + description + content`.

**THE RESIDUAL, PARTIALLY CLOSED 2026-08-03 (Task 15 / REQ-B14 — the G2 named residual "the known quoted-header parse quirk is fixed in a parse shared with workstreams-emit," docs/plans/gated-pipeline-master-2026-08.md Task 15).** Through 2026-07-30 this paragraph said *"any quoted header that STARTS A LINE — with arbitrary leading whitespace, including fence or tab indentation — anywhere within the first 5 lines of the JOINED text will emit a real `task_started`."* That was true THEN. `_extract_nl_attribution` now applies two additional guards, closing two of the three measured shapes: (1) **column-0 only** — the header line itself may carry NO leading whitespace at all (previously tolerant of arbitrary indentation); (2) **fence-precedence rejection** — a header whose immediately preceding line (within the window) opens or continues a markdown code fence (```` ``` ```` / `~~~`) is quoted content, not a live dispatch. Neither guard touches `NL_ATTRIBUTION_MAX_LINE` itself (the width-tightening deferral below is UNCHANGED). Measured against the shipping parser POST-fix: a 4-space-indented paste **no longer emits** (RPL7f, was EMIT, now closed); a 2-line preamble + fenced paste **no longer emits** (RPL7d, was EMIT, now closed).

The remaining, narrower residual: **a quoted header that starts a line at COLUMN 0 with NO code fence immediately before it** — e.g. prose ending in a colon on the line directly above, then the header flush left with no fence markup at all (RPL7i's shape, still asserted EMIT deliberately) — still emits a real `task_started`. Closing THAT needs either tightening `NL_ATTRIBUTION_MAX_LINE` (still deferred, unmeasured cost — see below) or an out-of-band field the prose cannot forge (still unavailable from PreToolUse `tool_input`).
...
Same `plan=`/`task=` vocabulary `admission-lib.sh`'s `adm_admit` labels and `estate-registration-lib.sh`'s `reg_register` labels already use — one attribution vocabulary across all three surfaces, not a fourth shape.

**Parser + wiring** (`hooks/workstreams-emit.sh`):
- `_extract_nl_attribution <text>` parses the header, tolerant of absence (`attributed=0`, never guessed) and partial presence (plan without task still records `attributed=0` but retains the parsed plan value for diagnostic visibility — never silently drops parsed data).
- START trigger (`--on-builder-dispatch`, PreToolUse, fires in the DISPATCHING/parent session; also `--on-spawn` since 2026-07-30): the header is now the **SOLE** source of the `task_started` progress-log emission — not merely the preferred one. The free-text heuristic (`_extract_plan_slug`/`_extract_task_id`) was REMOVED from that path on 2026-07-30 (ROADMAP-FALSE-ETERNAL-RUNNING-01): scraping the prompt text meant a dispatch that merely MENTIONED `docs/plans/<slug>.md` or the words "Task 9" marked that task started, which is what kept the operator's cockpit chips green with nothing running. **No header ⇒ no `task_started`, ever** — honest silence, never a guess. The header is ALSO threaded into the governor ledger row (`adm_admit emit-feed plan=... task=... role=... attributed=...`) — the same row already written 1000+/day, now consumer-ready.
- START trigger, second condition — **replay gate** (2026-07-30): PreToolUse re-fires for EVERY historical Task/Agent tool call in the transcript (measured: 100 fires in 67s replaying 43h of dispatch history, five times in one afternoon). `task_started` and the provenance marker are therefore emitted only on the FIRST fire of a dispatch identity — `item_id = sha1(session|tool|title)`, whose first-seen record in a per-session DISPATCH-IDENTITY ledger is now load-bearing rather than merely observational. A re-dispatch under a distinct title is unaffected; an identical-title re-dispatch is treated as the same dispatch (documented, deliberate: it is indistinguishable from a replay at PreToolUse, and a missing green beats a false one). **THE IDENTITY LEDGER IS NOT THE CONCLUDE LEDGER** (corrected 2026-07-30): the spawn surface first keyed this gate on `opened-<sid>.jsonl`, which `--on-stop` deletes at every turn boundary and `--heartbeat` deletes on staleness — so the "first fire only" guarantee was erased one turn after being set and every later replay counted as a start again, i.e. this claim was FALSE on the spawn surface for as long as it was written down. A dispatch-identity SET must outlive the session; a conclude QUEUE must be cleared when drained. They are now separate files — `builder-<sid>.jsonl` and `spawn-<sid>.jsonl`, both append-only and deleted by no code path. Known defeats, disclosed: the same identities under a NEW `session_id` re-emit (measured 4 → 8), and deleting the identity ledger re-emits (1 → 2).
- The dispatch-provenance MARKER keeps the header-then-free-text resolution — deliberately. It feeds `pl_classify_session`'s correlation guard, not a user-visible claim, so a looser hint is the right trade there. It is replay-gated all the same.
- END trigger (`--on-stop`, fires in the STOPPING session — which may be a dispatched CHILD, e.g. a `plan-phase-builder` worktree session): reads the header from the STOPPING session's OWN transcript first user turn (`_stop_extract_nl_attribution`, via `transcript_path`) — since the dispatch prompt IS that session's first turn — and folds it into the `spawn-concluded` `ledger_emit` detail string, so start and end carry the same plan/task/role vocabulary.
- HONEST GAP: the START row's `session` is the DISPATCHING (parent) session's id; the END row's `session` is the DISPATCHED (child)'s own id — necessarily different ids, since a PreToolUse hook cannot know a not-yet-created child's session id. A consumer must join on plan+task+recency, NOT session id, for start/end pairing — see `docs/plans/fragments/attribution-server-fragment.md`. A `spawn-concluded` row with no matching START row within a reasonable window is its own honest class ("concluded, never observed starting"), not a join failure.

**Enforcement rung (constitution §10):** WARN-only, not a gate. `_run_on_builder_dispatch` logs a WARN line + a running unattributed-dispatch count (grep-derived from the append-only hook log, not a separate read-modify-write counter) whenever `attributed=0`; the dispatch is never blocked. Golden scenario: the operator sees the cockpit's "N running, unattributed to a task" banner while the session reports more agents in flight, with no surface able to arbitrate (2026-07-29, this convention's own motivating incident). Expected false-positive rate: dispatches genuinely outside any plan (ad hoc research, `Explore`, one-off debugging) are a real, non-trivial population — measured against the real `~/.claude/state/governor/ledger/` at build time: 0 of 1528 emit-feed rows carried any attribution (100% of that day's dispatches predate the convention), so the honest steady-state FP rate can only be measured after adoption, not predicted from a pre-convention baseline. Retirement condition: attribution rate >95% sustained for one month → consider promoting to a block (a SEPARATE decision, separately evidenced — a WARN count is not self-promoting).
...
## Solution-shape protocol (OD-024) — the layer sweep before any dispatch

**Origin:** operator directive 2026-08-04, verbatim: "I want for you to always be thinking about comprehensive solutions. I am finding that it has consistently been the case that the solutions you propose tend to assume very simple, single-threaded issues and propose simple, single-threaded solutions. I keep trying to push for comprehensive solutions. ... the best solution should really include the mechanical solution: better prompting for guiding the agents in the correct direction, better pattern definition, etc. There are multiple layers available to us for making the harness do the right thing. I want for us to be utilizing all of those layers, not just one." This generalizes the same failure the proactive-audit-loop section above closed at the VERIFICATION end (reactive-not-proactive) to the FIX-DESIGN end: an orchestrator that reaches for the first single-layer patch that makes a reported symptom stop is repeating the identical single-threaded pattern one step earlier in the pipeline.

**The standing obligation (not discretionary):** before dispatching a fix for ANY operator-reported defect, the orchestrator writes two things, in order, BEFORE the first builder dispatch:

1. **A CLASS STATEMENT.** Name the general failure class the reported defect is one instance of. "The cockpit fails to parse this plan's Status field" is an instance; "plan Status fields have no validated vocabulary" is the class. If you cannot state the class in one sentence, you have not looked hard enough yet — go back to the evidence, not straight to the fix.
2. **A LAYER SWEEP across six layers**, one line each:
   - **Schema/data model** — is the underlying data shape itself the gap (a missing enum, an unvalidated field, an ambiguous type)?
   - **Mechanical gate** — should a hook/validator refuse the bad state at write time, so the instance-class cannot recur even if every other layer regresses?
   - **Parser/renderer** — does the consuming code need to widen (never laxen a fail-loud honestly) to the corrected shape?
   - **Migration of existing estate** — does every existing instance across the whole estate (not just the one that got reported) need to move to the corrected shape?
   - **Doctrine/pattern** — does the standing convention need to say the corrected shape explicitly, so the next author does not have to reconstruct it from source?
   - **Agent prompt** — does the agent that PRODUCES this data (a plan-authoring builder, a template) need to be steered at generation time, so the mistake is prevented, not just caught downstream?

Each of the six layers gets **used or explicitly waived with a stated reason** — never silently skipped. "Instance-only" (fix the one reported case, touch nothing else) is a valid outcome ONLY when it is a named, justified exception ("this is a one-off typo, not a recurring shape — layers 2/4/5/6 waived, cost outweighs benefit for a single dead-end case"), never the unexamined default.

**The dispatch prompt MUST carry the class statement.** A builder dispatched on layer 3 (say, the parser) without knowing the class statement will happily patch the parser to accept one more freelanced string — solving the instance, reproducing the class. The class statement is what lets a builder scoped to a single layer still build toward the general fix instead of the narrow one.

**Anti-pattern this closes:** dispatching a single builder to "fix the parse error" with no class statement and no layer sweep — the exact single-threaded, instance-only shape the operator named as the recurring failure in the 2026-08-04 directive above. A fix that only widens the parser's accepted-value list is NOT a comprehensive solution even if it makes today's error message go away: the next freelanced Status value reproduces the identical symptom.

### Worked fixture — the status-vocabulary program (2026-08-04)

**Instance:** the cockpit renders `status unknown — plan parse failed (unrecognized Status: ...)` for ~11 plans in a sibling product repo (a separate harness-managed checkout on this machine; name withheld per the harness-hygiene denylist — see "Cross-repo worktree dispatch" above). Measured census at scope time (independently re-verified against the live trees, not just the dispatch-time claim): neural-lace's `docs/plans/deferred/` carried 13 plain `Status: DEFERRED` plus 1 `Status: DEFERRED (operator 2026-07-30: ...)` (14 files, confirmed by `grep -rn "^Status:" docs/plans/deferred/*.md`), `docs/plans/*.md` carried 3 `ACTIVE`, 5 `REFERENCE (spec appendix...)`-style prose, and 1 `NORMATIVE for Wave O builders (...)`; the sibling repo's live (non-archive, non-deferred) `docs/plans/*.md` carried 20 `ACTIVE`, 6 `DRAFT`, 1 `PROPOSED`, 2 `PROPOSAL` variants with prose, and 2 `DRAFT — <prose>` (confirmed by `grep -rlo "^Status: ACTIVE" docs/plans --include="*.md" | grep -v archive | grep -v deferred` run against that repo's own `docs/plans/`, and the matching greps for the other values). Every one of those free-text variants is a plan Status header an author typed by hand with no vocabulary to check it against.

**Class:** plan `Status:` fields have no validated vocabulary — any prose an author types is accepted at write time, and only the cockpit's parser (which correctly fails loud rather than guessing) surfaces the mismatch, per-plan, after the fact.

**The layer sweep, all six named:**
1. **Schema/data model** — the canonical enum (`DRAFT, PROPOSED, ACTIVE, COMPLETED, SUPERSEDED, DEFERRED, ABANDONED, REFERENCE`) plus a new optional `Status-note` header line immediately after `Status:`, so prose qualifiers (`(operator 2026-07-30: ...)`, `(spec appendix...)`) have a sanctioned home instead of being smuggled into the enum field itself.
2. **Mechanical gate** — a write-time check (plan-authoring / commit-time) that a plan's `Status:` value is drawn from the canonical enum, so an unrecognized 12th value cannot land again the same way the first 11 did.
3. **Parser/renderer** — `neural-lace/workstreams-ui/server/plan-parse.js` widens to recognize the canonical enum (and the `Status-note` line) WITHOUT laxening its fail-loud behavior on anything still outside the enum — the honest failure this program exists to keep, not remove.
4. **Migration of existing estate** — every one of the ~24 non-canonical Status lines measured above (neural-lace + the sibling repo) gets rewritten to a canonical value with prose moved to `Status-note`; `PROPOSAL` → `PROPOSED`, `NORMATIVE`-prose → `REFERENCE` are the two migration-time renames the orchestrator decided rather than leaving ambiguous.
5. **Doctrine/pattern** — this section, plus the planning doctrine's own Status-field convention, name the canonical enum as the standing vocabulary (not left implicit in a parser's source) — and this same 2026-08-04 dispatch is itself the first landing of that layer.
6. **Agent prompt** — the plan-authoring surface (template header, `plan-phase-builder` guidance) is steered toward the canonical enum at generation time, so a new plan is written correctly the first time instead of relying on the parser to catch a freelanced value after the fact.

This program is the reference fixture for the protocol above: an instance-only fix here would have been "teach `plan-parse.js` to also accept `REFERENCE (spec appendix...)`" — solving 1 of the ~24 measured cases and leaving the class (freelanced prose, no validated vocabulary) fully intact for case #25. The orchestrator dispatching this fix ran two tracks from the same class statement: a builder track scoped to layers 1/3/4 (the vocabulary, the parser, and the estate migration), and this doctrine/directive track (Agent 2) scoped to layer 5 (this section, plus the OD-024 register entry). **Honest state at the time this section was written (PROVEN, not assumed):** `neural-lace/workstreams-ui/server/roadmap-routes.js`'s `KNOWN_PLAN_STATUS_TOKENS` was directly re-read and still reads `{ ACTIVE, COMPLETED, DEFERRED, ABANDONED, SUPERSEDED }` — `DRAFT`, `PROPOSED`, and `REFERENCE` are NOT yet in it, so layer 3 had not landed as of this doctrine track's commit; `adapters/claude-code/templates/plan-template.md` line 2 still reads `Status: ACTIVE` with no enum listed, so layer 6 had not landed either. Layers 1/3/4/2/6 are the builder track's deliverable, not this one's — do not cite them as shipped without re-running the same checks (`grep -n "KNOWN_PLAN_STATUS_TOKENS" neural-lace/workstreams-ui/server/roadmap-routes.js`; `grep -n "Status:" adapters/claude-code/templates/plan-template.md`; search for a status-vocabulary gate under `adapters/claude-code/hooks/`) against current master.
