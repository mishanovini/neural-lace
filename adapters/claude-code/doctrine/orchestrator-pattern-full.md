...
**The format.** Every dispatch prompt (`Task`/`Agent`/`Workflow` tool calls, and `mcp__ccd_session__spawn_task`) MUST include a line anywhere in the prompt text:
...
- START trigger (`--on-builder-dispatch`, PreToolUse, fires in the DISPATCHING/parent session): the header, when present, is AUTHORITATIVE and takes precedence over the free-text heuristic for the `task_started` progress-log emission + dispatch-provenance marker (`_emit_dispatch_provenance`); it is ALSO threaded into the governor ledger row (`adm_admit emit-feed plan=... task=... role=... attributed=...`) — the same row already written 1000+/day, now consumer-ready.
