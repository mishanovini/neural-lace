...
**The format.** Every dispatch prompt (`Task`/`Agent`/`Workflow` tool calls, and `mcp__ccd_session__spawn_task`) MUST **OPEN WITH** this line — it must start a line, and sit within the first `NL_ATTRIBUTION_MAX_LINE` (default 5) lines of the prompt:
