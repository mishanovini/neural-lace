...
**THE OUTPUT CONTRACT, UPDATED (2026-08-05, model-fallback mechanization).** `dispatch-directives.sh`'s stdout is now THREE things in a fixed order, not two: (1) the `NL-ATTRIBUTION: plan=... task=... role=...` header, (2) a `model: <tier>  # <cause>` line — role_category maps the dispatch's role (builder → category `build`; verifier/reviewer/advocate → category `review`) to a `config/model-policy.json` chain, and `model-availability.sh resolve` walks it, skipping any tier a REAL observed death has marked exhausted (never an assumption). `<cause>` is either `primary tier` (chain[0] was fresh) or `fallback (skipped: <tier1>[,<tier2>...])` — a closed-vocabulary tier list ONLY, never the free-text reason a marker was stored with (2026-08-06 fix: the raw reason field used to be echoed here, and since a stored reason can itself carry arbitrary text, that was a self-referential-injection surface — a real spend-limit death's reason text, once echoed into the NEXT dispatch's prompt, could seed a false exhaustion mark on the tier that ran THAT dispatch, one hop later). Then (3) a blank line and the register-computed `Directives:` citation block. A caller pasting this script's output verbatim gets the correctly-attributed header AND the correct model to dispatch on, with no judgment call and no reason text that could carry anything unexpected.
