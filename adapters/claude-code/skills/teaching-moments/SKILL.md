...
See `~/.claude/doctrine/teaching-moments.md` for when capture qualifies + the file-format spec.
...
2. **Confirm it qualifies.** Apply the four criteria from `~/.claude/doctrine/teaching-moments.md` — substantive pushback, meaningfully better revised position, generalizable lesson, enough context surviving.
...
- Don't auto-capture every minor correction. The bar in `~/.claude/doctrine/teaching-moments.md` is "0-1 captures per session is normal; 3+ means the bar is too low."
...
- `~/.claude/doctrine/teaching-moments.md` — the capture rule (when to write, format spec)
