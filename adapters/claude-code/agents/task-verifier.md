...
**Ledger event — automatic, nothing for you to do (2026-08-04, operator directive: "check off the check boxes in two places (plan file and ledger)").** The SAME `plan-edit-validator.sh` invocation that authorizes your Edit in step 3 also appends a `flip-verdict` row to the shared signal ledger (`$HOME/.claude/state/signal-ledger.jsonl`) — plan slug, task id, verdict, confidence, verifier, and an evidence pointer — BEFORE it returns `exit 0` and lets your Edit proceed. You never call anything extra; there is no separate step to remember or skip. The plan-file checkbox is the human-reviewable PROJECTION of that ledger EVENT, not a second source of truth. If you ever need to confirm a task's event landed (e.g. debugging a flip that seemed to succeed but the flip-verdict row looks absent), run `bash adapters/claude-code/scripts/verify-event-audit.sh --task <plan-file> <task-id>` — it reports `HAS_EVENT_ON_RECORD` or the honest `NO_VERIFICATION_EVENT_ON_RECORD` (never fabricated either way). Known, disclosed limitation: the emit only fires when your checkbox-flip Edit is actually authorized by this hook, which requires the task id on the checkbox line to match the hook's own dotted-letter `TASK_ID` regex (`[A-Z]+\.[0-9]+(\.[0-9]+)*`, e.g. `A.1`, `SE.4.1`) — a bare-numeric id (`7.`) or a fused id (`SE3`) never reaches this function at all today (HARNESS-GAP-62, `docs/backlog.md`); this is a pre-existing gap in the authorizer itself, not something this ledger addition introduces or is expected to work around.
