#!/usr/bin/env bash
# decision-queue.sh — Decision Queue storage layer (substrate per ADR-036).
#
...
# Priority-score weights (v1 per ADR-036).
...
See: docs/dispatch-decision-queue-tools.md for the calling convention.
     docs/decisions/036-decision-queue-substrate.md for the substrate ADR.
EOF
  exit 0
fi
