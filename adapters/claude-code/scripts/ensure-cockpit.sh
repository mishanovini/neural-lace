...
# lifecycle instead of the `ConversationTreeUI-AutoStart` logon scheduled
# task (operator preference: no scheduled-task sprawl). This script is
...
#   3. Launcher path resolved from the MAIN checkout via
#      hooks/lib/nl-paths.sh's nl_main_checkout_root() — NEVER a worktree,
...
run_ensure() {
  ...
  if ! declare -F nl_main_checkout_root >/dev/null 2>&1; then
    _ec_log "tolerate-absent: hooks/lib/nl-paths.sh unavailable (nl_main_checkout_root undefined) — skipping cockpit ensure"
    return 0
  fi
  ...
  if [[ -z "$main_root" ]]; then
    _ec_log "tolerate-absent: nl_main_checkout_root unresolved (not in a git checkout?) — skipping cockpit ensure"
    return 0
  fi

  local launcher="$main_root/neural-lace/workstreams-ui/scripts/launch-gui.ps1"
  if [[ ! -f "$launcher" ]]; then
    _ec_log "tolerate-absent: launcher not found at $launcher — skipping cockpit ensure"
    return 0
  fi
...
EOF
  ...
  mkdir -p "$s2_dir"
  ( cd "$s2_dir" && ENSURE_COCKPIT_UNAME_OVERRIDE="MINGW64_NT" ENSURE_COCKPIT_LOG_PATH="$s2_log" HARNESS_SELFTEST=1 \
      "$bash_bin" -c "source '$self_abs'; run_ensure" )
  ...
  ( cd "$s3_dir" && ENSURE_COCKPIT_UNAME_OVERRIDE="MINGW64_NT" ENSURE_COCKPIT_LOG_PATH="$s3_log" HARNESS_SELFTEST=1 \
      "$bash_bin" -c "source '$self_abs'; run_ensure" )
  local s3_out; s3_out="$(cat "$s3_log" 2>/dev/null || true)"
  _ck_contains "S3 Windows + missing launcher tolerates absent" "$s3_out" "launcher not found"
  ...
  ( cd "$s4_dir" && ENSURE_COCKPIT_UNAME_OVERRIDE="MINGW64_NT" ENSURE_COCKPIT_LOG_PATH="$s4_log" HARNESS_SELFTEST=1 \
      "$bash_bin" -c "source '$self_abs'; run_ensure" )
  ...
  ( cd "$s5_dir" && ENSURE_COCKPIT_UNAME_OVERRIDE="MINGW64_NT" ENSURE_COCKPIT_LOG_PATH="$s5_log" HARNESS_SELFTEST=1 \
  ...
  if [[ "$s6_wt_ok" -eq 1 ]]; then
    ( cd "$s6_wt" && ENSURE_COCKPIT_UNAME_OVERRIDE="MINGW64_NT" ENSURE_COCKPIT_LOG_PATH="$s6_log" HARNESS_SELFTEST=1 \
        "$bash_bin" -c "source '$self_abs'; run_ensure" )
...
EOF
  ...
  s7_start=$(date +%s)
  ( cd "$s7_dir" && ENSURE_COCKPIT_UNAME_OVERRIDE="MINGW64_NT" ENSURE_COCKPIT_LOG_PATH="$s7_log" \
  ...
  ( cd "$s3_dir" && ENSURE_COCKPIT_UNAME_OVERRIDE="MINGW64_NT" ENSURE_COCKPIT_LOG_PATH="$tmp/s9b.log" HARNESS_SELFTEST=1 \
      "$bash_bin" "$self_abs" >/dev/null 2>&1 )
  s9b_rc=$?
