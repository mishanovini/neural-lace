...
STUBEOF
  ...
  # ================================================================
  # Scenario 18 (task cold-reader-lint, constitution §3 amendment 53d3bee):
  ...
  ( cd "$REPO" && git init -q -b master 2>/dev/null || (git init -q && git checkout -q -b master 2>/dev/null); \
    git config core.hooksPath ""; git config user.email t@example.com; git config user.name T; git config commit.gpgsign false; \
    echo seed > seed.txt; git add -A; git commit -q -m seed )
  T18=$(_write_transcript "$tmproot/s18" $'Decision needed: ship tonight?\nOption A or option B.\nMy pick: A.\n\nDONE: nothing to report')
  RC18=$(_run_dispatcher "$HOOKS" "$REPO" "$T18" "sess-s18")
  _expect "cold-reader-lint-warn-never-blocks" "$RC18" "0"
  ...
  # ================================================================
  # Scenario 19: a WELL-FORMED decision block (repo-path anchor + a table
  ...
  ( cd "$REPO" && git init -q -b master 2>/dev/null || (git init -q && git checkout -q -b master 2>/dev/null); \
    git config core.hooksPath ""; git config user.email t@example.com; git config user.name T; git config commit.gpgsign false; \
    echo seed > seed.txt; git add -A; git commit -q -m seed )
  T19=$(_write_transcript "$tmproot/s19" $'Decision needed: ship the change in adapters/claude-code/scripts/needs-you.sh tonight?\n| Option | What happens |\n|---|---|\n| Ship | goes live now |\n| Wait | ships Monday |\nMy pick: ship.\n\nDONE: nothing to report')
  RC19=$(_run_dispatcher "$HOOKS" "$REPO" "$T19" "sess-s19")
  _expect "cold-reader-lint-well-formed-block-never-blocks" "$RC19" "0"
  ...
  # ================================================================
  # Scenario 20: ordinary prose with NO decision-block markers at all is
  ...
  ( cd "$REPO" && git init -q -b master 2>/dev/null || (git init -q && git checkout -q -b master 2>/dev/null); \
    git config core.hooksPath ""; git config user.email t@example.com; git config user.name T; git config commit.gpgsign false; \
    echo seed > seed.txt; git add -A; git commit -q -m seed )
  T20=$(_write_transcript "$tmproot/s20" $'Just shipped the fix, no decision needed here.\n\nDONE: nothing to report')
  RC20=$(_run_dispatcher "$HOOKS" "$REPO" "$T20" "sess-s20")
  _expect "cold-reader-lint-ordinary-prose-never-blocks" "$RC20" "0"
  if ! grep -q '"gate":"stop-verdict-dispatcher".*cold-reader-lint' "$SIGNAL_LEDGER_PATH" 2>/dev/null; then
  ...
  # ================================================================
  # Scenario 21: a WARN never appears in the combined BLOCK message and
  ...
  ( cd "$REPO" && git init -q -b master 2>/dev/null || (git init -q && git checkout -q -b master 2>/dev/null); \
    git config core.hooksPath ""; git config user.email t@example.com; git config user.name T; git config commit.gpgsign false; \
    echo seed > seed.txt; git add -A; git commit -q -m seed )
  T21=$(_write_transcript "$tmproot/s21" $'Decision needed: ship tonight?\nMy pick: yes.\n\ntrailing off with no marker at all')
  RC21=$(_run_dispatcher "$HOOKS" "$REPO" "$T21" "sess-s21")
  _expect "cold-reader-lint-warn-plus-real-gap-still-blocks-on-the-real-gap" "$RC21" "2"
  if grep -q "marker-format" "$tmproot/last-stderr.txt" 2>/dev/null \
