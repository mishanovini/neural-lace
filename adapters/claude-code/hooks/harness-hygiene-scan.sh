...

{
  ...
  cat "$MATCHES_TMP"
  echo ""
  echo "Remedy: fix the content, or add the file to is_exempt() in this scanner"
  echo "(with a comment naming the exemption class) and stage both in the same"
  echo "commit. (--no-verify cannot bypass this scan's PreToolUse wiring and is"
  echo "prohibited without operator say-so — constitution §7.)"
