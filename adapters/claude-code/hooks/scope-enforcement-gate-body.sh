# scope-enforcement-gate-body.sh — the full mechanism (helpers, --self-test,
# main hook logic) for scope-enforcement-gate.sh, SOURCED (never executed
# directly) by that thin entry file. Full spec/header (trigger, target-repo
# resolution, active-plan discovery, diff comparison, waivers, exit codes)
# lives in scope-enforcement-gate.sh's own header — read that first. This
# split exists so the overwhelmingly common case — a Bash/PowerShell command
# with no "commit" substring at all — never pays the cost of parsing this
# ~2400-line file: the entry's cheap pre-filter decides BEFORE sourcing this
# body (Gate Philosophy Law, R3.4, harness-execution-redesign-2026-08
# Task 2).
#
# ============================================================
...
# ============================================================
# _SEG_SELF_DIR resolves to THIS file's directory (adapters/claude-code/
# hooks/), byte-identical to the entry's directory (both live in hooks/),
# so lib/* sourcing below is unaffected by the entry/body split.
...
source "$_SEG_SELF_DIR/lib/gate-contract-lib.sh" 2>/dev/null || true
# GC_MODE: computed once from the entry's frozen argv[1] (_SEG_ARGV1, set by
...
if [[ "${1:-}" == "--self-test" ]]; then
  ...
  # Resolve to the ENTRY file (scope-enforcement-gate.sh), not this body
  # file: every scenario below re-invokes SELF_TEST_HOOK as a real
  # subprocess exactly as a PreToolUse call would, and only the entry has
  # the cheap pre-filter + --self-test/--check dispatch. _SEG_ENTRY_PATH is
  # set by the entry before it sources this body (see that file);
  # BASH_SOURCE[0] here would resolve to THIS body file, testing the wrong
  # artifact — that fallback exists only for a direct `bash
  # scope-enforcement-gate-body.sh --self-test` debugging invocation.
  SELF_TEST_HOOK="${_SEG_ENTRY_PATH:-$(cd "$(dirname "${BASH_SOURCE[0]}")" 2>/dev/null && pwd)/$(basename "${BASH_SOURCE[0]}")}"
...
'
  ...
  # ---- Scenario 43/44: Gate Philosophy Law (R3.4) — --check pre-flight ----
  # Same _run_scenario fixture shape as scenarios 1/3, but invokes the ENTRY
  # with --check as argv[1] and captures stderr, proving --check and enforce
  # reach the identical decision (one shared call site, GC_MODE-branched
  # only at the very end) rather than two implementations that could drift.
  _run_scenario_check() {
    local label="$1" plan_body="$2" staged_csv="$3"
    local repo="$TMPROOT/$label"
    mkdir -p "$repo"
    (
      cd "$repo" || exit 99
      ...
      printf '%s' "$input" | "$SELF_TEST_BASH" "$SELF_TEST_HOOK" --check >/dev/null 2>stderr.txt
      echo $? > rc.txt
    )
    cat "$repo/rc.txt" 2>/dev/null || echo 99
  }

  S43_RC=$(_run_scenario_check s43 "$PLAN_NORMAL" "src/foo.ts,unrelated/file.ts")
  S43_ERR=$(cat "$TMPROOT/s43/stderr.txt" 2>/dev/null || echo "")
  if [[ "$S43_RC" == "1" ]] \
     && [[ "$S43_ERR" == *"WOULD BLOCK (--check pre-flight, not enforced)"* ]] \
     && [[ "$S43_ERR" == *"[GATE:WHAT]"* ]] && [[ "$S43_ERR" == *"[GATE:WHY]"* ]] \
     && [[ "$S43_ERR" == *"[GATE:FIX]"* ]] && [[ "$S43_ERR" == *"[GATE:ESCAPE]"* ]] \
     && [[ "$S43_ERR" == *"UPDATE THE PLAN"* ]]; then
    echo "self-test (43) check-mode-would-block-same-fields: PASS" >&2
    PASSED=$((PASSED+1))
  else
    echo "self-test (43) check-mode-would-block-same-fields: FAIL (rc=$S43_RC, expected 1 + WOULD BLOCK + 4 fields + three-option text)" >&2
    FAILED=$((FAILED+1))
  fi

  S44_RC=$(_run_scenario_check s44 "$PLAN_NORMAL" "src/foo.ts,src/bar.ts")
  if [[ "$S44_RC" == "0" ]]; then
    echo "self-test (44) check-mode-clean-pass-allowed: PASS" >&2
    PASSED=$((PASSED+1))
  else
    echo "self-test (44) check-mode-clean-pass-allowed: FAIL (rc=$S44_RC, expected 0)" >&2
    FAILED=$((FAILED+1))
  fi
...
{
  echo "================================================================"
  echo "$(gc_header "SCOPE ENFORCEMENT GATE — COMMIT BLOCKED" "$GC_MODE")"
  echo "================================================================"
  echo ""
  gc_block \
    "commit blocked — staged files fall outside the active plan's declared scope" \
    "out-of-scope commits silently expand work beyond the plan, eroding the scope discipline that makes plans verifiable" \
    "append an In-flight-scope-update bullet to the active plan, or open a new plan claiming the file(s), then re-stage and re-commit — see the three options below" \
    "none — this is a PreToolUse hook that pattern-matches the git commit command before git runs, so no git commit-hook bypass affects it; the three options below are the only remediations"
  echo ""
...
# JSON decision for Claude Code — enforce mode only; --check never claims a
# real decision on stdout.
