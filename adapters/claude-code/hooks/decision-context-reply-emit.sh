...
_resolve_gui_state_path() {
  ...
  if mr=$(_main_repo_root) && [[ -n "$mr" ]]; then
    for d in workstreams-ui; do
      c="$mr/neural-lace/$d/state/tree-state.json"
      if [[ -f "$mr/neural-lace/$d/state/state.js" ]]; then printf '%s' "$c"; return 0; fi
      c="$mr/$d/state/tree-state.json"
      if [[ -f "$mr/$d/state/state.js" ]]; then printf '%s' "$c"; return 0; fi
    done
  fi
