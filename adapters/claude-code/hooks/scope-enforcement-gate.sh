...
'
  ...
  # ---- Scenarios 17-20: HARNESS-GAP-41 trailing-slash pattern semantics ----
...
'
  ...
  # ---- Scenario 17: PASS — `foo/` matches bare path `foo` (the gitlink case) ----
  RC=$(_run_scenario s17 "$PLAN_GITLINK" "gitlink-dir")
  if [[ "$RC" == "0" ]]; then
    echo "self-test (17) trailing-slash-matches-bare-path: PASS (gitlink case)" >&2
    PASSED=$((PASSED+1))
  else
    echo "self-test (17) trailing-slash-matches-bare-path: FAIL (rc=$RC, expected 0)" >&2
    FAILED=$((FAILED+1))
  fi

  # ---- Scenario 18: PASS — `foo/` still matches paths under foo (existing behavior preserved) ----
  RC=$(_run_scenario s18 "$PLAN_GITLINK" "gitlink-dir/nested/file.ts")
  if [[ "$RC" == "0" ]]; then
    echo "self-test (18) trailing-slash-matches-nested-path: PASS (existing behavior preserved)" >&2
    PASSED=$((PASSED+1))
  else
    echo "self-test (18) trailing-slash-matches-nested-path: FAIL (rc=$RC, expected 0)" >&2
    FAILED=$((FAILED+1))
  fi

  # ---- Scenario 19: FAIL — `foo/` must NOT match `foobar` (false-positive guard) ----
  RC=$(_run_scenario s19 "$PLAN_GITLINK" "gitlink-dirbar")
  if [[ "$RC" == "2" ]]; then
    echo "self-test (19) trailing-slash-does-not-match-substring-prefix: PASS (correctly blocked)" >&2
    PASSED=$((PASSED+1))
  else
    echo "self-test (19) trailing-slash-does-not-match-substring-prefix: FAIL (rc=$RC, expected 2)" >&2
    FAILED=$((FAILED+1))
  fi

  # ---- Scenario 20: FAIL — `foo/` must NOT match `foo-extra` (hyphen-extended false-positive guard) ----
  RC=$(_run_scenario s20 "$PLAN_GITLINK" "gitlink-dir-extra")
  if [[ "$RC" == "2" ]]; then
    echo "self-test (20) trailing-slash-does-not-match-hyphen-extended: PASS (correctly blocked)" >&2
    PASSED=$((PASSED+1))
  else
    echo "self-test (20) trailing-slash-does-not-match-hyphen-extended: FAIL (rc=$RC, expected 2)" >&2
    FAILED=$((FAILED+1))
  fi
