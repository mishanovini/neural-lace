...
# ============================================================
# Additive auto-trace via static-trace.sh (non-blocking)
# ============================================================
# After the plan-declared chain has been verified (above), also run the
# auto-detect static trace against the branch's modified files. This
# catches structural breakage in files the plan didn't explicitly
# declare in its Wire checks block (e.g., a broken chain introduced by
# a tangential edit). Additive only — does NOT block the checkbox flip.

STATIC_TRACE_SCRIPT=""
for cand in \
  "$REPO_ROOT/adapters/claude-code/scripts/static-trace.sh" \
  "$HOME/.claude/scripts/static-trace.sh"; do
  if [[ -f "$cand" ]]; then
    STATIC_TRACE_SCRIPT="$cand"
    break
  fi
done

if [[ -n "$STATIC_TRACE_SCRIPT" ]] && git -C "$REPO_ROOT" rev-parse HEAD >/dev/null 2>&1; then
  BASE_REF=""
  for cand in origin/master origin/main master main; do
    if git -C "$REPO_ROOT" rev-parse --verify "$cand" >/dev/null 2>&1; then
      BASE_REF=$(git -C "$REPO_ROOT" merge-base HEAD "$cand" 2>/dev/null || echo "")
      [[ -n "$BASE_REF" ]] && break
    fi
  done
  if [[ -z "$BASE_REF" ]]; then
    BASE_REF=$(git -C "$REPO_ROOT" rev-parse HEAD~5 2>/dev/null || echo "")
  fi

  if [[ -n "$BASE_REF" ]]; then
    MODIFIED_FILES=$(git -C "$REPO_ROOT" diff --name-only "$BASE_REF" HEAD 2>/dev/null \
      | grep -E '^(src/|app/|components/|lib/|pages/|prisma/)' \
      | head -50)
    if [[ -n "$MODIFIED_FILES" ]]; then
      ST_OUT=$(echo "$MODIFIED_FILES" \
        | (cd "$REPO_ROOT" && bash "$STATIC_TRACE_SCRIPT" 2>&1))
      ST_EXIT=$?
      ST_TOTAL=$(echo "$ST_OUT" | grep -E "Total files:" | head -1 | sed -E 's/.*: *//')
      if [[ $ST_EXIT -eq 0 ]]; then
        echo "[wire-check] auto-trace (additive): ${ST_TOTAL:-0} modified file(s) scanned, all chains OK" >&2
      else
        echo "[wire-check] auto-trace (additive): BROKEN chain(s) in modified files (does NOT block)" >&2
        echo "$ST_OUT" | grep -E "^[[:space:]]*BROKEN:" | head -5 \
          | sed 's/^/[wire-check]   /' >&2
        echo "[wire-check]   For full report: bash $STATIC_TRACE_SCRIPT --git-diff" >&2
      fi
    fi
  fi
fi
