...
# drift at session start. Drift detection component (c).
#
...
# (NEVER block) if their master SHAs disagree. Pairs to a sibling poller
# (`scripts/check-cross-repo-drift.sh`, component b) for the periodic backstop.
#
# Behavior:
...
if [ "$poller_rc" -eq 1 ]; then
  ...
  echo "SHAs. The next push via 'sync.sh' will surface the same drift in its"
  echo "post-push verification. To reconcile:"
  echo "  1. Decide which side is canonical for the divergent commits."
  echo "  2. Cross-merge / cherry-pick the missing commits onto that side."
  echo "  3. Run 'sync.sh' to push the unified state to all remotes."
  echo "================================================================"
  echo ""
fi
