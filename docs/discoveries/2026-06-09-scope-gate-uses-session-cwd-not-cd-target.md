...
(This builder session is itself rooted in a sibling product repo's worktree, not the
...
Option C (A + B), resolved by shipped mechanisms rather than a single sit-down:
**A (target-repo resolution in the gate)** landed 2026-06-10 for
`scope-enforcement-gate.sh` (HARNESS-GAP-47) and 2026-06-12 for the same class
in `plan-lifecycle.sh` (HARNESS-GAP-49 / FM-032 — see "Second instance of the
class" below). **B (sync from master, never a dirty working copy, with
verification)** is substantially covered by `session-start-auto-install.sh`
(ADR 048, shipped 2026-06-02): it syncs live `~/.claude/hooks/` + `~/.claude/scripts/`
from the freshest FETCHED `origin/master` ref — not the working tree —
master-wins-with-backup, content-compared. Residual accepted: a mid-session
manual `cp` from a dirty canonical copy can still regress the live mirror until
the next SessionStart self-heals it from master (exactly how the 2026-06-09
regression instance was corrected). Status flipped pending → implemented
2026-06-12 on that basis.

## Implementation log

- 2026-06-10: option A landed for the scope gate — `scope-enforcement-gate.sh`
  now resolves the effective target repo from the command itself (`git -C`
  flags, `cd`/`Set-Location` prefixes, fallback to process cwd) and the matcher
  covers the PowerShell tool; see the "Target-repo resolution (HARNESS-GAP-47,
  2026-06-10)" header section in the hook + self-test scenarios 26-31.
- 2026-06-12: the same fix pattern applied to `plan-lifecycle.sh` (see "Second
  instance of the class" below) — repo root derived from the edited file's path
  (`git -C "$(dirname <file>)" rev-parse --show-toplevel`), all git operations
  `git -C <that-root>`; cross-repo self-test scenario 10 added; live mirror
  synced byte-identically. `plan-status-archival-sweep.sh` audited and confirmed
  clean (its `archive_plan()` already roots per-file via
  `git -C "$plans_dir" rev-parse --show-toplevel`; its `$PWD` scan is
  cwd-by-design for a SessionStart hook).
- 2026-06-12: the class is now catalogued as **FM-032** in
  `docs/failure-modes.md` ("Hook resolves target repo from session cwd, not
  from the operated-on subject") with the per-hook detection pattern (cross-repo
  self-test fixture) and prevention recipes for path-subject vs command-subject
  hooks.
- Option B (hook-sync pulls from master + byte-identity verification) remains
  open — this discovery stays `pending` for that half.
...
2. **Salvage builder (wf_195f11bc-e9c) repro**: Bash-tool commits in a sibling product repo's worktrees blocked against
...
## Second instance of the class (appended 2026-06-12): plan-lifecycle.sh cross-repo mis-archival

Observed 2026-06-11: a builder session rooted in neural-lace flipped `Status: ACTIVE → COMPLETED`
(Edit tool) on a plan file inside a sibling product repo's worktree
(`<sibling-product>-worktrees/<wt>/docs/plans/<plan-slug>.md`).
`plan-lifecycle.sh` (PostToolUse) resolved its archival repo root from the hook PROCESS cwd
(neural-lace), not the edited file's repo: the cross-repo path fell through `to_repo_relative()`
unchanged, `git ls-files --error-unmatch` ran in the WRONG repo and reported "untracked", and the
plain-`mv` fallback physically moved the plan into `neural-lace/docs/plans/archive/` and staged it
there — deleting it from the sibling repo's worktree that owned it (repaired by hand at the time).

Same root shape as the scope gate's bug, on a path-subject hook instead of a command-subject hook.
Fixed 2026-06-12: repo root derived from `tool_input.file_path`
(`git -C "$(dirname <file>)" rev-parse --show-toplevel`), every git operation runs
`git -C <that-root>`, and the fix upgraded cross-worktree archival from the history-losing
plain-`mv` fallback to a true staged `git mv` rename (the tracked-check now runs in the right
repo). Regression-locked by self-test scenario 10 (plan fixture in a second temp repo while cwd
stays in the first). Class entry: FM-032.
