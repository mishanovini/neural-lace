...
type: failure-mode
status: implemented
resolution: shipped ad1ebd0 (item-text-set/branch-retitled events, selftest P19) + ws-ui-residuals-2026-06-10 plan COMPLETED (Task 3 emitted the 10 corrections). Marked 2026-07-12 overnight session per state audit.
auto_applied: false
originating_context: R5 builder (wf_6e2aa9d7-be2) mojibake diagnosis. 30 U+FFFD chars on node <product>-launch-sprint-2026-05-29 (10 items × text/background/the_ask). Corruption is upstream of canonical — the audit log carries EF BF BD at ingest (the 2026-06-09T07:23 migration read already-lossy text, likely em-dashes mangled by an earlier encoding hop).
...
or re-authorable from the <product> launch-sprint docs.
