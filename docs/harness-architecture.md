...
| `pre-push-test-gate.sh` **(2026-04-24)** | Global git pre-push hook (NOT a Claude hook) | Blocks pushes to `master`/`main` without a green-test receipt for the current HEAD SHA. Opt-in per repo via `<repo>/.claude/pre-push-test-gate.enabled` marker. Receipt written by `scripts/record-test-pass.sh` after a successful local test run. Pairs with server-side required-status-check branch protection — local gate blocks regressions leaving the dev box, remote gate blocks them landing on master. Escape: `PT_PUSH_NO_TEST_GATE=1 git push` or `git push --no-verify`. |
...
| `harness-reviewer.md` | default | Adversarial review of harness rule/hook/agent changes. Default verdict is REJECT. Used before landing any `~/.claude/` modification. Emits class-aware feedback per the six-field contract per defect. **Extended (2026-04-24, Gen 5)** with Step 5: enforcement-gap proposal review — verdicts PASS / REFORMULATE / REJECT against five generalization checks (section presence, class-vs-instance, existing-rule-review honesty, proposal proportionality, testing-strategy class coverage). Applied to draft proposals under `docs/harness-improvements/` produced by `enforcement-gap-analyzer`. |
...
| `enforcement-gap-analyzer.md` **(Gen 5, 2026-04-24)** | default | Reads runtime acceptance failures (FAIL artifact + plan + transcript + hooks-fired list) and produces a draft harness-improvement proposal at `docs/harness-improvements/<YYYY-MM-DD>-<class-slug>.md`. Mandatory existing-rule review before proposing anything new — missed-catches by an existing rule trigger AMENDMENT, not addition. Output has five required sections (Class of failure / Existing rules that should have caught this / Why current mechanisms missed this / Proposed change / Testing strategy). Generalization-disciplined: rule must fire on ≥ 2 distinct sibling instances, not just the named instance. Hands off to `harness-reviewer` (Step 5 extended remit) for verdict before landing. Closes the meta-loop: runtime advocate FAIL → analyzer proposal → reviewer verdict → committed harness improvement. |
...
| `record-test-pass.sh` **(2026-04-24)** | Called by the builder after a successful local test run in a repo with the pre-push-test-gate enabled | Writes a `.claude/state/test-receipt-<branch-slug>-<head-sha>.txt` file in the current repo with the token `TESTS_PASSED_FOR_SHA=<sha>`. The receipt is the signal `pre-push-test-gate.sh` looks for to authorize a push of that SHA to master/main. The script does NOT re-run tests — caller is responsible for only invoking after a green test run. |
