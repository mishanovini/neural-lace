...
| `plan-closure-validator.sh` **(HARNESS-GAP-16, Phase 1d-H, 2026-05-05; tagged-for-retirement during Tranche F)** | PreToolUse `Edit\|Write` | Blocks irreversible `Status: ACTIVE → COMPLETED` transitions on `docs/plans/<slug>.md` files unless five mechanical closure checks pass: (a) every `## Tasks` checkbox flipped; (b) every task ID has a `Verdict: PASS` evidence block; (c) `## Completion Report` section populated; (d) every `Backlog items absorbed:` slug reconciled in `docs/backlog.md`; (e) `SCRATCHPAD.md` mtime within 60 minutes AND mentions the slug. **As of Tranche E (2026-05-05) the deterministic procedure `adapters/claude-code/scripts/close-plan.sh close <slug>` is the primary closure path — it produces evidence that satisfies these checks in seconds with zero agent dispatches.** Tranche F audits whether this hook's preconditions remain load-bearing under the new lighter regime; many become obsolete once mechanical evidence and per-task verification routing are in place. Until retirement, the hook continues to fire as a safety net during the transition. Has `--self-test` flag exercising 10 scenarios. |
