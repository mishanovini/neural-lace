...
| **Wire-check gate (2026-05-11)** | `Edit\|Write\|MultiEdit` | Blocks `[ ]`→`[x]` flips on `docs/plans/*.md` tasks declared (or defaulting to) `Verification: full` whose plan body contains a `**Prove it works:**` sub-block, unless the session's evidence substrate proves the scenario was executed. Three acceptable proof shapes: (1) prose evidence file `Wire check executed:` / `Prove-it-works run:` block ≥ 80 chars under matching `Task ID:` section; (2) structured `<plan-slug>-evidence/<task-id>.evidence.json` with at least one `runtime_evidence` entry + at least one `passed: true` mechanical check; (3) canonical carve-out `Wire check: n/a — <reason ≥ 30 chars>`. Mechanical/contract tasks exempt. Companion to `plan-reviewer.sh` Check 13 (plan-time author-side enforcement). Has `--self-test` flag exercising 6 scenarios. Wired AFTER `plan-edit-validator.sh`. |
