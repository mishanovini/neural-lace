...
| **Wire-check gate (2026-05-11)** | `Edit\|Write\|MultiEdit` | On checkbox flip in a plan file: if the flipped task declares `Verification: full` (or unmarked, defaulting to full) and has a `**Wire checks:**` block, parses the block's `→` arrows and verifies each backtick-quoted file path exists relative to repo root AND each non-file token (function name, SQL fragment, API route) appears via `grep -F` in at least one of the linked files. Requires ≥ 2 verified arrows OR a `n/a — <reason ≥ 30 chars>` carve-out. BROKEN arrows or insufficient coverage block the flip. After the plan-declared chain is verified, additively invokes `static-trace.sh` against the branch's `git diff --name-only` modified files (Next.js page/api/component/lib/Prisma chain auto-detection) and logs the summary or BROKEN list to stderr — non-blocking. Wired AFTER `plan-edit-validator.sh` (so evidence-first protocol fires first) and BEFORE `spec-freeze-gate.sh`. 9 self-test scenarios. |
