...
| `plan-closure-validator.sh` **(HARNESS-GAP-16, Phase 1d-H, 2026-05-05)** | PreToolUse `Edit\|Write` | Blocks irreversible `Status: ACTIVE → COMPLETED` transitions on `docs/plans/<slug>.md` files unless five mechanical closure checks pass: (a) every `## Tasks` checkbox flipped; (b) every task ID has a `Verdict: PASS` evidence block in `<slug>-evidence.md` or in the plan's `## Evidence Log`; (c) `## Completion Report` section populated with substantive Implementation Summary (≥ 20 chars body); (d) every `Backlog items absorbed:` slug reconciled in `docs/backlog.md` (under a "Recently implemented" / "Completed" / "Resolved" / "ABSORBED" heading OR with a `(deferred from ` marker); (e) `SCRATCHPAD.md` mtime within 60 minutes AND mentions the slug. Non-COMPLETED terminal flips (DEFERRED/ABANDONED/SUPERSEDED) are NOT gated. Exits 2 with structured stderr + JSON `{"decision": "block", ...}` when any check fails. Pairs with the `/close-plan` skill which walks the orchestrator through closure mechanically so the right path is the path of least resistance. Wired AFTER `plan-edit-validator.sh` (so plan-file edits flow through evidence-first first) and BEFORE `spec-freeze-gate.sh`. Has `--self-test` flag exercising 10 scenarios (all-checks-pass-allows, missing-checkbox-blocks, missing-evidence-blocks, missing-completion-report-blocks, unreconciled-backlog-blocks, stale-scratchpad-blocks, transition-to-DEFERRED-allows, transition-to-ABANDONED-allows, transition-to-SUPERSEDED-allows, non-Status-edit-passes-through). Closes the 2026-05-05 plan-stranding gap (a plan ACTIVE for two days with all 5 task checkboxes empty despite all 5 tasks shipped on master). |
