...
- [ ] AUTO: decision waiting on operator — "Your machine-local ~/.claude/settings.json wires a hook script that no longer exists, so two entries fail with exit 127 on every agent dispatch. I cannot fix it myself: ~/.claude/settings.json is operator config and agents are barred from editing it without the grant-local-edit mechanism. This needs one command from you." (needs-you `NY-1785468126-b004`, tier 2, session `unknown`) — see NEEDS-YOU.md
- [ ] AUTO: decision waiting on operator — "**Decision needed:** Should `adapters/claude-code/doctrine/` (89 files of harness rule prose) be added to the review-record surface, so a doctrine change needs a `harness-reviewer` PASS before it can be pushed to master?" (needs-you `NY-1785468489-b4eb`, tier 1, session `unknown`) — see NEEDS-YOU.md
<!-- AUTO:END -->
