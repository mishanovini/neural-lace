...
- [x] AUTO: question waiting on operator — "GitHub Actions billing on the Pocket-Technician org is broken: every workflow job on every push/PR fails at start with 'recent account payments have failed or your spending limit needs to be increased' (first seen master push 2026-07-30T01:00Z, run 30504375449). Because branch protection requires these checks, NO PR can merge estate-wide until billing is fixed. Concretely waiting: PR #106 (docs-only split-view incident finding + worktree-scope-write-gate proposal, branch already updated to master). Action when you can: GitHub org Settings -> Billing & plans -> fix payment/limit, then re-run failed checks; merge with: gh pr merge 106 --squash --delete-branch (repo has auto-merge disabled, so I could not arm it)." (needs-you `NY-1785373628-cb99`, tier untiered, session `defceaa5-37e4-4753-beaf-288aa1fcbab7`) — see NEEDS-YOU.md
<!-- AUTO:END -->
