...
**Structural note (2026-08-02, Gate Philosophy Law retrofit, harness-execution-redesign-2026-08 Task 2, first half):** `hooks/lib/gate-contract-lib.sh` is new — the shared structured-block-message emitter (`gc_block`: WHAT/WHY/FIX/ESCAPE) and `--check` pre-flight mode helper (`gc_mode`/`gc_header`/`gc_exit_code`) that `scope-enforcement-gate.sh`, `pre-commit-gate.sh`, and `concurrent-ownership-gate.sh` now source. `scope-enforcement-gate.sh` and `concurrent-ownership-gate.sh` also each split into a thin dispatcher (the same filename, still wired in `settings.json.template`) plus a new sourced `*-body.sh` file carrying the full mechanism, so the common non-matching Bash/PowerShell event never parses/sources the body at all — `hooks/scope-enforcement-gate-body.sh` and `hooks/concurrent-ownership-gate-body.sh` are new files with no independent `settings.json.template` wiring of their own (they are sourced, never invoked directly, except by their entry's own `--self-test`). `pre-commit-gate.sh` was retrofitted in place (no split — its relevance filter already lives one layer up in `settings.json.template`'s PreToolUse command). manifest.json entries for these three gates' new contract fields are still pending (see the follow-up note immediately below for what landed since, and this plan's own evidence file for the full remaining-scope list — the doctor gate-message lint and `harness-hygiene-scan.sh`'s own retrofit are still not built).

**Structural note (2026-08-03, THE GATED PIPELINE, gated-pipeline-master-2026-08 Task 1 — walking skeleton, design `docs/designs/gated-pipeline-master-2026-08-03.md` §4):** `hooks/lib/review-chain-lib.sh` is new — the ONE parser/validator (REQ-B6) for the `## Review Chain` block every pipeline-stage artifact will carry, implementing the design's three validity rules (record parse; three-way anchor match with plan-side canonicalization + a WARN-only in-flight-visibility check; dispatch-ledger cross-check with a pre-ledger exemption). `hooks/dispatch-chain-gate.sh` is new — the G2 (plan → build) gate, but SCOPED to a `--check <plan-file>` pre-flight mode ONLY in this task; there is deliberately NO `settings.json.template` PreToolUse wiring yet and no `subagent_type` trigger logic — both are Task 17 (REQ-B8's full form + the D-15 three-variant acceptance-bar demo). Fixtures live under `adapters/claude-code/tests/fixtures/review-chain/` (see that directory's own `README.md`) including the dispatch-ledger row schema `{subagent_type, model, ts, session_id, artifact_ref}` — the shared contract Task 15's `workstreams-emit.sh` writer must also honor. manifest.json entries for both new files are orchestrator-integrated at merge (this plan's stated builder-locked deviation from the design's "same-commit" letter), not part of this commit.

**Structural note (2026-08-03, THE GATED PIPELINE, gated-pipeline-master-2026-08 Task 11 — operator-directives register, design `docs/designs/gated-pipeline-master-2026-08-03.md` §4, REQ-B1+B4):** `hooks/lib/directives-register-lib.sh` is new — the ONE parser (`dr_validate`/`dr_entry_ids`/`dr_get_field`/`dr_surface_matches`/`dr_entries_for_files`) for the canonical register `config/operator-directives.json` (20 seeded entries: 18 general + 2 `operator_only`), which every future carriage consumer (`plan-reviewer.sh` Check 21, `scripts/dispatch-directives.sh`, `hooks/doctrine-jit.sh`'s register walk — all Task 14/20) will share, per the design's "ONE parser lib" rule. `scripts/gen-directives-view.sh` is new — a `gen-architecture-doc.sh`-style generator (`--gen`/`--check`/`--self-test`) producing the human-readable `docs/operator-directives.md` (regenerated, never hand-edited) FROM the register JSON; the two are deliberately independent (the generator reads the JSON directly rather than through the lib, matching how `gen-architecture-doc.sh` reads `manifest.json` directly). `hooks/decisions-index-gate.sh` gained an additive, WARN-only ADR forward-guard (arch-L1): a staged `docs/decisions/` record asserting binding standing-rule language without a `register_ref` mention or an explicit "no standing rule" opt-out now WARNs (never blocks — the file's existing record↔index atomicity check is untouched). Fixtures live under `adapters/claude-code/tests/fixtures/directives-register/` (see that directory's own `README.md`) — the shared round-trip contract Task 20 reuses, same convention as Task 1's `tests/fixtures/review-chain/`. `manifest.json` entries for the new files are orchestrator-integrated at merge (this plan's stated builder-locked deviation from the design's "same-commit" letter), not part of this commit.

**Structural note (2026-08-02, Gate Philosophy Law retrofit, harness-execution-redesign-2026-08 Task 2, deferred remainder — partial):** `hooks/lib/workaround-sensor-lib.sh` is new — the operator's workaround-as-sensor law, mechanized: `ws_record <gate> <bypass_kind> <command_fingerprint> [detail]` appends one JSONL line (fields: ts, gate, session, bypass_kind, command_fingerprint, detail) to `~/.claude/state/workaround-sensor.jsonl`, spawn-free on its normal (directory-already-exists) path (bash builtins only — `printf '%(...)T'` for the timestamp, no `date`/`tr`/`jq`/`git`). `hooks/lib/gate-contract-lib.sh` gained one small additive function, `gc_escape_used <gate> <bypass_kind> <fingerprint> [detail]`, which lazily sources the new lib and calls `ws_record` — the ready-to-call mechanism for "using a sanctioned escape hatch gets ledgered automatically." NOT done in this pass (explicitly out of scope for the dispatch that built this): wiring `gc_escape_used` into the five retrofitted gates' own waiver-honored call sites (e.g. `concurrent-ownership-gate-body.sh`'s `_guard_slug`, which still only calls the older generic `lib/signal-ledger.sh` `ledger_emit ... "waiver" ...`) — those gate body files were off-limits for this dispatch. `hooks/plan-edit-validator.sh` also gained the structured WHAT/WHY/FIX/ESCAPE fields + a `--check` read-only pre-flight mode, but SCOPED to its two genuinely WARN-only checks (`check_docs_impact_warn`, `check_backlog_absorption_warn` — both already documented never-block) plus a cheap `docs/plans` substring relevance pre-filter ahead of its `jq` calls. This file's SEPARATE, actually-blocking mechanism (checkbox-flip-without-evidence / `Status: COMPLETED`-without-evidence, `exit 1`, manifest.json `"blocking": true`, ADR 059 D4 "deliberately unwaivable") was deliberately left untouched — see this task's build report for the reasoning. Still not built: `harness-hygiene-scan.sh`'s own retrofit, the doctor gate-message lint, and JIT pre-warning hints.
