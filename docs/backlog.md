...
## Open — orchestrator-prime (2026-06-08)
- Agent-upgrade staged apply: 24 research-grounded proposals in docs/reviews/agent-upgrades/. NOT applied (behavior-defining). Review then apply in staged batches with a contract-preservation gate (each rewrite keeps six-field class-aware output + PROVEN/HYPOTHESIZED + evidence discipline). Downstream-product journey deep-dive + parallel doc review follow.
