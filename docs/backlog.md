...
- **HARNESS-GAP-27 — `scope-enforcement-gate.sh` blind to merge-commit semantics; forces manual plan-edits during routine merges** (added 2026-05-14). Surfaced during a downstream-project PR-merge session (UX-audit follow-up work). The gate iterates ACTIVE plans currently visible in `docs/plans/*.md` and rejects any staged file not claimed by an active plan's `## Files to Modify/Create` section. When a feature branch merges master back in, master's concurrent commits pull in files from plans that were ACTIVE on master but are now archived (their parent plans got `Status: COMPLETED` and auto-archived to `docs/plans/archive/`). The gate doesn't see archived plans, so master's pulled-in files appear unclaimed, blocking the merge commit. **Today's workaround:** author a session-scope "merge-resolution plan" that wildcard-claims everything master might have touched (`docs/**`, `src/**`, project-specific subtrees, etc.). This is fragile — the merge-plan author has to anticipate every directory master might have touched, and the gate-fires-once-fixes-once cycle adds friction (the originating PR hit it twice on a single merge: first miss was a migrations subtree from an auth-rebuild branch; second miss was a harness-config subtree from a deploy-mode commit). **Proper fix candidates:** (a) **Lightweight:** extend `_is_system_managed_path()` to honor a "merge-context allowlist" when `$(git rev-parse --git-dir)/MERGE_HEAD` exists — e.g., exempt project-managed subtrees and any other path patterns master controls procedurally during a merge. Similar pattern to the existing `docs/plans/archive/*` exemption. Concrete + small. (b) **More general:** when MERGE_HEAD exists, recognize merge-resolution context and check staged files against the UNION of all plans active on EITHER branch since the divergence point, not just plans currently active in HEAD. This requires walking both sides of the merge via `git log $(git merge-base HEAD MERGE_HEAD)..HEAD` and `..MERGE_HEAD` to enumerate every plan that was at one point active during the divergence window. Bigger change; needs ADR. (c) **Smallest:** add a sentence to the gate's stderr remediation message naming the "merge-resolution plan" pattern as the canonical solution + a worked example, so the friction is well-documented even if not removed. Effort: (a) ~1-2 hours; (b) ~4-6 hours including self-test scenarios for merge-aware diff walking; (c) ~15 min. Priority: P2 (operational friction during merges, not blocking work — workaround exists). Risk: low. Companion to GAP-20 (retrofit discipline → mechanism) since the merge-plan workaround is itself a discipline that mechanism could replace.
