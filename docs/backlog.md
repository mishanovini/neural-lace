...
## HARNESS-GAP-39 — parallel Dispatch sessions on the same module collide on version-label + plan-slug + items, requiring per-PR reconcile cycles (added 2026-05-20)

Observed across 5 parallel PRs on `neural-lace/conversation-tree-ui/`
landing within ~36 hours (#10 items 14-18, #11 items 14-23 superset,
#13 v1.1.2 drop-item-20, #15 v1.1.3 tnode-root, #12 v1.1.2 items 25-39
— THIS session). Each Dispatch worktree session independently:
(a) picked the next "v1.1.x" version label with no cross-session reservation,
(b) authored a plan at the same `docs/plans/conv-tree-ui-v1.1.X-polish.md`
path for unrelated work,
(c) implemented an overlapping subset of items under different numbering
(my "item 25" = their merged "item 22"; my "item 26" / "item 38" =
their later v1.1.4 re-do "items 26/38" with different visuals; their
"v1.1.3 item 38" = a .li-kind label restyle directly contradicting
Misha's actual item 36 spec).

**Impact in this session alone** (4-PR-deep collision chain on PR #12):
- 3 separate `git merge origin/master` reconcile cycles, each bringing
  master-inherited files OUTSIDE my plan's `## Files to Modify/Create`
  (`config/projects.js`, `server/server.js`, `state/backfill-details.js`,
  ADRs 033/034, the OTHER v1.1.2 plan archived, etc.).
- Each cycle required `scope-enforcement-gate` Option-1 remediation
  (extending `## In-flight scope updates` with ~24+30 explicit path
  bullets per cycle to document master-inherited paths).
- One round needed a fresh `conv-tree-spawn-waiver` because a
  task-verifier sub-agent Agent spawn carried no Pin-1 candidate token
  (NL-FINDING-010 class — gate-respect Option 2).
- B15 (NL-FINDING-012) regressed AND was fixed-upstream by #13's
  in-flight backfill repointing — discovered only via the reconcile.
- A PR-template-check workflow change on master (`81d40a6 fix(pr-template-check):
  only check placeholder in selected sub-section`) added a stricter
  `## What mechanism would have caught this?` parent-heading requirement
  that wasn't in the template when I authored PR #12's body; required
  two PR-body edits + two CI re-runs.
- Net: PR #12 grew to 7 commits (4 substantive + 3 merge/reconcile) and
  the plan's `## In-flight scope updates` section ballooned to ~50
  master-inherited path bullets — most of the section by line count is
  collision-documentation, not v1.1.2 work.

**Class.** parallel-Dispatch-session-version-and-slug-and-item-collision-
on-hotspot-module (same family as NL-FINDING-013 but the recurring
META-pattern, not a single instance).

**Why this matters as a HARNESS-GAP, not just a finding.** NL-FINDING-013
captures one collision. The pattern that *generates* collisions is
structural: Dispatch's worktree spawn flow has no awareness of other
in-flight worktrees on the same module/version label. Every session
independently picks the next-available slug and item-number range,
guaranteeing collisions on any module that gets 2+ parallel sessions.
conversation-tree-ui has 5 in 36 hours; the cost is borne by whoever
merges last (PR #12 in this case).

**Proposed remediations** (ranked, none acted on unilaterally):
- A. **Cheapest** (Pattern-class, no code): a session-start convention
  that any new conv-tree-ui-vN.N plan does `git log --all --oneline
  --since=7d -- docs/plans/conv-tree-ui-vN.N-* docs/plans/archive/conv-tree-ui-vN.N-*`
  and surfaces colliding labels to the operator (Misha) BEFORE creating
  the plan. Could be a `pre-plan-version-collision-check.sh` PreToolUse
  hook on `Write` to `docs/plans/<slug>.md` that warns + names the
  collision but doesn't block (this session would have surfaced PR #11/#13
  to me at plan-creation time).
- B. **Mechanism**: a per-machine "in-flight version reservation"
  surface (file at `~/.claude/state/version-reservations/<repo>.json`
  enumerating ACTIVE-or-recently-merged plans across all worktrees on
  this machine) that the SessionStart hook displays — same shape as
  `discovery-surfacer.sh`.
- C. **Upstream ask** (NOT in our control): Dispatch session-spawn
  could pass `--reserve-version <label>` so the spawning lead and
  spawned child agree on a non-colliding slug.

**Companion**: NL-FINDING-013 (single-instance), HARNESS-GAP-35
(Dispatch worktree accumulation — same "spawn is Anthropic-side" theme),
HARNESS-GAP-28 (Dispatch spawner env-var convention — same theme).

**Not blocking** (Misha's "keep driving" directive on PR #12 had me
reconcile-and-ship rather than pause for a structural fix), but worth
surfacing because the cost scales: a 6-item PR became a 15-item PR
requiring 3 reconciles; a 30-item PR at the same collision density
becomes effectively unmergeable.
