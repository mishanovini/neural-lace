...
**A — Scanner self-test repair (P1).** `harness-hygiene-scan.sh --self-test` FAILS today: it asserts an exemption for `docs/plans/foo.md` that was deliberately removed from the implementation. A regression check that itself fails masks real regressions. Fix: update the self-test to match current exemption logic (post-removal). Effort: ~15 min.

**B — Scanner exemption logic vs `.gitignore` allow-list reconciliation (P1).** Scanner directory-exempts `docs/decisions/*`, `docs/reviews/*`, `docs/sessions/*`. `.gitignore` now allow-lists committed entries from those dirs (`!docs/decisions/[0-9][0-9][0-9]-*.md`). The full-tree scan returns "exit 0" only because exempt paths aren't checked — codename leakage in committed decision/review files would not be detected. Fix: tighten scanner exemption to match the gitignore allow-list (i.e., scan committed decision/review/session files; exempt only the gitignored ones). Re-run full-tree scan after fix; expect new findings to surface. Effort: ~30 min + however long the new findings take to fix.
...
**D — Missing `adapters/claude-code/schemas/automation-mode.schema.json` (P3).** `public-release-hardening.md` Task 6.1 lists this file in "Files to Create" but it doesn't exist on disk. The example.json + the gate hook ship; the schema doesn't. Either write the schema or remove the file from the plan's claimed scope. Effort: ~30 min (schema authoring + tests).

**E — Close out `public-release-hardening.md` unchecked tasks 1.2, 4.2, 5.3, 6.1 (P3).** Either complete them or document explicit deferral with rationale. Plan's completion report falsely claimed "all phases all PASS" — bring the bookkeeping in line with reality. Effort: ~15 min for the bookkeeping; per-task effort varies depending on which path is chosen for each.

**F — Close out `harness-quick-wins-2026-04-22.md` Phase A Task 1 (P2).** Live `~/.claude/settings.json` has no `effortLevel` field. Either complete (one-line edit + commit + plan-task checkbox flip via task-verifier) or document explicit deferral with the rationale that the per-project hook (`effort-policy-warn.sh`) covers most of the value and the global default is acceptable. Effort: ~30 sec for the edit; ~5 min for proper task-verifier flip + bookkeeping.
