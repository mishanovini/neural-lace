...
## HARNESS-GAP-39 — conv-tree-state-gate over-fires on read-only / no-conversation-tree spawns; write-evidence.sh resolves plan-dir to cwd on `--plan <slug>` (added 2026-05-19)

P2. Two real gaps surfaced while baking the FM-catalog convention (plan `fm-catalog-harness-architecture`, merged `3e5ecad`):

1. **conv-tree-state-gate over-fire (FM-024 recurrence; companion to NL-FINDING-010).** `conversation-tree-state-gate.sh` blocked all three `mcp__ccd_session__spawn_task` bootstrap-chip dispatches in a harness-docs session that has no conversation-tree state file and produces no branch (read-only/no-tree spawn). The gate's named remediation (a substantive `.claude/state/conv-tree-spawn-waiver-*.txt`) was applied per `gate-respect.md`/`git-discipline.md` Rule 3 and the recurrence recorded on FM-024 per its Recovery field. This is now the **third** confirmed instance of the class (two prior docs sessions + this one) and overlaps the already-filed NL-FINDING-010 ("gate blocks bare Task/Agent sub-agent dispatches carrying no Pin-1 token; needs a gate-side fix"). **Proposed fix (per NL-FINDING-010 candidates + FM-024 Prevention):** gate-side exemption for spawns with no write-capable tools / no branch / read-only `subagent_type`, OR accept the `sp-<sha1>` hash node the emitter already writes. Until the gate-side fix lands, the substantive waiver is the correct per-occurrence path (but a reflexive waiver IS the bug report — that is why this entry exists).
2. **write-evidence.sh plan-dir resolution.** `write-evidence.sh capture --plan <slug>` (slug, not path) wrote the structured `.evidence.json` artifacts to `<cwd>/<slug>-evidence/` (repo root) instead of `<plan-dir>/<slug>-evidence/` (`docs/plans/`), so `close-plan.sh` could not find them and blocked all 7 tasks until the artifacts were regenerated with `--plan docs/plans/<slug>.md` (full path). The close-plan remediation message itself suggests the slug form (`--plan <slug>`), so the two are inconsistent. **Proposed fix:** when `--plan` is a bare slug, resolve plan-dir via `find-plan-file.sh <slug>` (archive-aware) rather than defaulting to cwd; OR make close-plan's remediation message emit the full path form. Minor (clean workaround exists) but it cost a full evidence-regeneration cycle.
