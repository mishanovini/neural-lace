...
So the problem is structural, not cosmetic: the medium (a single linear chat) cannot represent the actual shape of the work (a branching tree of open threads and pending Q&A). Everything below is a **consequence** of that root mismatch.
...
Misha explicitly does not do manual git/code work — the system and Dispatch drive execution. So the problem is not "give Misha tools to manage code"; it is "give Misha a durable, visible, navigable model of the *conversation tree* — its open threads, pending decisions, and unanswered questions — with the things he is the bottleneck on surfaced and linked back to where they came from."
