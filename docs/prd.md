...
- **OQ-10. (GAP — raised by Misha at Stage B, explicitly not yet thought through) Two-list vs one-list information architecture.** Should the GUI separate (a) a **backlog** list of items Misha wants to work on but has not started a session for, from (b) an **action-items** list of follow-ups required on in-flight work? Or is it one unified list with a state field? This is a UX-architecture question Misha raised verbatim ("If this GUI can be used as a to-do list, should we separate a backlog items list … from the list of action items that I need to follow up on?") and explicitly has not resolved. No AI lean recorded (per the no-synthesis discipline — this is Misha's to decide). Decide in: Stage F resolution (resolve / defer-with-rationale / halt), informed by the eventual ux-designer review.
