# ADR 046 — Feature-Completion Criteria Gate (eight criteria, mechanical, with a periodic audit backstop)
...
### Layer 2 — `feature-completion-audit.sh` (periodic backstop)

A project-generic script (no project identifiers — it ships under `adapters/`, scanned by the hygiene gate) that walks `docs/plans/**` for `Status: COMPLETED` items and verifies each post-fact against the same eight criteria using real repository state (git log for PR/merge SHAs, `docs/support/*.mdx` presence for contractor-facing pages classified via an optional page-registry, migration mentions, deploy/acceptance markers). It writes a report and exits non-zero if gaps exist. This is the catch-all for what the gate's phrase-trigger misses.

**The two layers are intentionally complementary, not redundant.** The gate favors false-negatives (a real shipment with unusual phrasing slips through) precisely *because* the audit is the thorough net. A gate that fired on every session would be noise; a phrase-scoped gate plus a state-grounded audit is the right division of labor — fast in-session backstop + thorough post-fact net.
...
- **Hardcode the audit to the downstream project.** Rejected on harness-hygiene grounds. The script ships in the kit (`adapters/`) and must carry no downstream-project identifiers. It is parameterized by `--project <path>` and reads generic `docs/plans/`, `docs/support/`, `docs/backlog.md` conventions; the per-machine default project path lives outside the kit.
