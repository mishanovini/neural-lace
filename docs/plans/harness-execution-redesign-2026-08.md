...
- 2026-08-02 (task 3 build): `docs/handoffs/2026-08-02-stage1-pending-integration.md` — new file, same OWED pattern as the stage-0 handoff: verbatim `manifest.json` entries (6) + `doctrine/harness-dev.md` section for the orchestrator to land at merge (task 3's dispatch kept both builder-locked, same constraint as task 1)
- 2026-08-02 (task 3 build): `docs/harness-guide.md` — a second new subsection ("Central maintenance core") introducing `scripts/nl-maintenance.sh` + the two platform adapters, same docs-freshness-gate rationale as task 1's bullet above
- 2026-08-02 (task 3 build): `adapters/claude-code/config/schedule-manifest.json` — bumped to schema_version 2 (`script_args`, `managed_by`, `legacy_task_name`, `retired_installer_note` fields; corrected the `session-heartbeat` entry's `script` field, which schema_version 1 pointed at the wrong file — see the task-3 handoff's "plan-spec ambiguities resolved" item 3 for the trace; added a new `doctor-verdict-refresh` entry). This file was already a declared task-1/task-3 create/modify target; the schema bump is an amendment within that same declared scope, noted here per the plan's own "amend rather than leave inconsistent" convention.
- 2026-08-02 (task 3 build): `neural-lace/workstreams-ui/server/maintenance-pane.selftest.js` — new file, standalone self-test for the new `/api/pane/maintenance-budget` endpoint (matches this codebase's existing per-concern `*.selftest.js` convention rather than folding into the much larger `server.selftest.js`)
- 2026-08-02 (task 3 build): `adapters/claude-code/hooks/session-start-digest.sh` was NOT modified, deviating from this task's own `Files to Modify/Create` table entry. Reasoning (full detail in the task-3 handoff's ambiguity item 4): `session-start-digest.sh` already had a pre-existing doctor-verdict cache (`refresh_doctor_cache`/`feed_doctor`, predating this task) that already avoided recomputing on every SessionStart — the genuine gap Task 3 closed was that `harness-doctor.sh --quick` invoked DIRECTLY never consulted that cache, which is fixed in `harness-doctor.sh` itself (reusing the same cache file, not a new one). Building a second, whole-digest snapshot mechanism on top of an already-adequate existing one would have been R3.3 anti-bloat duplication for no measured benefit; `pressure.json` (the other snapshot invariant 3 names) already exists via `health-tick.sh`'s pre-existing tick and needed no changes once `health-tick.sh` became an `nl-maintenance` job.
- 2026-08-02: `adapters/claude-code/hooks/scope-enforcement-gate-body.sh`, `adapters/claude-code/hooks/concurrent-ownership-gate-body.sh`, `adapters/claude-code/hooks/harness-hygiene-scan.sh` — task 2 deferred-remainder build, call-site wiring pass: `gc_escape_used`/`ws_record` now fire at every REAL escape-hatch call site found in these three files (closing the gap the prior bullet above left open). Wired: (1) `scope-enforcement-gate-body.sh`'s rebase/merge-resolution full-skip (`SKIP_REASON` branch, the gate's only actual exemption since its 2026-05-04 waiver removal) — `bypass_kind "exemption-list"`, alongside the pre-existing `EXEMPT_LOG` audit line, self-test scenario 45 added (50/50 passing, up from 49/49); (2) `concurrent-ownership-gate-body.sh`'s three waiver-honored sites (`_guard_slug` plan-slug + branch, and the `git worktree remove` waiver check) — `bypass_kind "waiver-file"`, alongside the pre-existing `ledger_emit "waiver"` calls, self-test scenario 22 added (22/22 passing, up from 21/21); (3) `harness-hygiene-scan.sh`'s structured-waiver honor site (`_hhs_is_waived`) — this gate is not yet retrofitted onto `gate-contract-lib.sh` (that retrofit is separately deferred), so it sources `workaround-sensor-lib.sh` directly and calls `ws_record` rather than `gc_escape_used`, `bypass_kind "waiver-file"`, self-test scenario W6 added (all scenarios green, incl. new W6). Every honor site fires unconditionally regardless of `--check`/enforce mode, matching the pre-existing sibling ledger call's own (also unconditional) behavior at each site — no new mode-awareness invented. Deliberately NOT wired: `pre-commit-gate.sh` and `backlog-plan-atomicity.sh` have no in-process bypass-HONORING code path — every block site's ESCAPE field in both files is either "none" (dead end, fix the real problem) or points to an operator editing an ALLOWLIST in a different, unrelated config file (`knip.config.ts`, `scripts/audit-event-coupling.ts`) that this gate's own runtime never reads/branches on, or (`backlog-plan-atomicity.sh` only) `git commit --no-verify`, an EXTERNAL bypass this PreToolUse script's own process cannot observe (if `--no-verify` actually skips this hook chain, the script never runs to record anything; empirically it does not skip it — the PreToolUse regex still matches "git commit" regardless of flags — but that is a pre-existing, out-of-scope claim in the block message this dispatch did not touch). Static allowlist categorization (scope-enforcement-gate's `docs/plans/archive/*`-class system-managed paths, `harness-hygiene-scan.sh`'s `is_exempt()`) was deliberately left unwired too — those are durable, plan-time, always-on categorizations a human never discretionarily invokes, not the discretionary "I need to route around this block right now" workaround the operator's law targets; ledgering every routine archival/exempt-path commit would flood the sensor with non-signal.
- 2026-08-02 (workstreams-ui cockpit-server subprocess-storm fix, dispatched directly, not a numbered plan task): `neural-lace/workstreams-ui/server/state-watch.js` — new file, the PUSH fast path (invariant: derive materialized state once, never recompute per request/poll). fs.watches the on-disk state feeding the six `nl <sub> --json` panes (heartbeats, signal-ledger.jsonl, needs-you ledger, doctor-cache.json, obs-costs-cache.json, remote-ledgers, docs/backlog.md — each resolved via the SAME env-var overrides the real derivation reads) and triggers a debounced (default 2s, capped at 15s under sustained churn) `cache.refreshAll()` on a REAL change, instead of a fixed clock tick. Root-caused: the cockpit's DeriveCache (existing, Wave O task O.4) already had single-flight/timeout/serve-last-good — but its `refreshAll()` trigger was a pure 30s timer, independent of whether anything had changed; `nl.sh` itself already carries a spawn-cascade circuit-breaker naming this exact 30s poll as the amplification source (`adapters/claude-code/scripts/nl.sh` lines 83-86, pre-existing, not touched by this fix). `OBS_WATCH_DISABLED=1` / `OBS_WATCH_DIRS` / `OBS_WATCH_DEBOUNCE_MS` / `OBS_WATCH_MAX_WAIT_MS` are the sandboxing/tuning knobs.
- 2026-08-02 (same fix): `neural-lace/workstreams-ui/server/state-watch.selftest.js` — new file, dedicated self-test (25/25 passing) measuring REAL subprocess spawns (not just in-process trigger calls) for: idle = zero spawns, one file change = exactly one refresh cycle, a 20-event burst = exactly one refresh cycle (not 20x), 5 concurrent `refreshAll()` calls dedupe to one cycle (single-flight), a failed refresh serves last-good with no retry storm, `derived_at` advances on a push-triggered refresh, `OBS_WATCH_DISABLED`/missing-directory/env-override handling.
- 2026-08-02 (same fix): `neural-lace/workstreams-ui/server/derive-cache.js` — `DeriveCache`'s own timer (`cache.start()`'s `setInterval`) is repurposed from the primary refresh trigger to the low-frequency ANTI-ENTROPY FLOOR only (default raised 30s -> 5 minutes, `ANTI_ENTROPY_FLOOR_DEFAULT_MS`, still overridable via the existing `OBS_REFRESH_MS`) — it now only heals a missed fs-watch event or an unwatched state producer; single-flight/timeout/serve-last-good/two-lane-concurrency were already present and are unchanged.
- 2026-08-02 (same fix): `neural-lace/workstreams-ui/server/server.js` — requires `state-watch.js`; starts TWO watch groups in the existing `server.listen()` success callback (same single-instance-guard timing as `cache.start()`/`auditor.start()`): group 1 (nl-subcommand state) triggers `cache.refreshAll()`; group 2 (the `nl-maintenance.sh` dashboard-snapshot directory, harness-execution-redesign task 3's new pane, landed on master while this fix was in flight — see the coordinator's mid-task correction) triggers `broadcastRefresh()` directly WITHOUT re-triggering the six nl-subcommand spawns, since `buildMaintenancePane()` is already a pure materialized-snapshot disk read, never a spawn. `/api/health` gained a `push_watch` block (invariant 5: health = output freshness, made visible — watched-dir counts, events-seen, triggers-fired per group).
- 2026-08-02 (same fix): `neural-lace/workstreams-ui/server/server.selftest.js`, `neural-lace/workstreams-ui/server/maintenance-pane.selftest.js` — added `OBS_WATCH_DISABLED=1` to each suite's existing env-sandboxing block (alongside the pre-existing `AUDITOR_DISABLED=1`), matching convention: without it, `server.js`'s new watch groups would fs.watch REAL `~/.claude/state` paths and/or this real repo's `docs/` directory (via `deriveLib.mainRepoRoot()`, not sandboxed by either suite's tmp-dir overrides) during the test run. Both suites re-verified green after the change (server.selftest.js: all scenarios through S26b pass; a PRE-EXISTING, reproduced-on-unmodified-baseline flaky crash in the later ask/dispatch-provenance fixture section, S23+, is unrelated — see this task's build report). `maintenance-pane.selftest.js`: 9/9 passing, unchanged from before this fix.
