...
- 2026-07-29: `adapters/claude-code/attic/lib/waiver-purpose-clause.sh` — M6 FIX (NEW). Fourth re-source shim completing the set NL-FINDING-018 created; without it the archived gate fell through to its pre-pin-f bare-existence waiver check. Self-test 29/2 -> 31/0 on BOTH bash 3.2.57 and 5.3.15
- 2026-07-29: `adapters/claude-code/config/selftest-sweep-exclusions.txt` — M6 EXCLUSION LEDGER (NEW). Path-keyed, one root-caused reason per entry, two entries: the attic decision-context gate and the attic workstreams extract-pending writer
- 2026-07-29: `adapters/claude-code/scripts/selftest-sweep-exclusions.sh` — M6 READER (NEW). --list is the sweep-runner API M5 must call; --check runs C1 path-exists / C2 reason-states-root-cause / C3 never-silence-a-live-surface; --verify runs C4 still-fails. Self-test 22/22 on BOTH interpreters
...
- 2026-07-29: `adapters/claude-code/scripts/selftest-sweep-exclusions.sh` — M6 FIX ROUND 2 (S11 flake). Root cause PROVEN: `attic/decision-context-gate.sh`'s own ST1 scenario drives `hooks/lib/stop-hook-retry-guard.sh` with a fixed session-id, and that library persists its counter at a CWD-relative `.claude/state/stop-hook-retries-decision-context-gate-st1.txt` never sandboxed by S11's caller — every prior self-test run through this script wrote into the REAL repo's own `.claude/state/` (measured: count=93 after ~90 repeated runs this investigation), and once the counter crosses `RETRY_GUARD_THRESHOLD` (3) the child's own block scenario silently downgrades (exits 0) instead of blocking, independent of whether this is really its 1st or 93rd invocation — a textbook cross-invocation shared-state leak, exactly the class the verifier's suspects list named. S11 (and do_verify, same class) now injects a fresh `RETRY_GUARD_STATE_DIR`/`HARNESS_SELFTEST_DIR` per child invocation, removing the dependency entirely. Self-diagnosing instrumentation added (ships regardless of future trigger): on any stale detection, prints the offending path, its rc, and a stdout+stderr tail. Discriminating mutation (transcript in the M6 fix commit / builder report): temporarily reverted just the isolation with a purpose-built fixture reproducing the same retry-guard pattern, in the REAL S11 loop — run 1-2 correctly PASS (count 1,2), run 3 goes RED with `got '1' want '0'` (the EXACT verifier-reported signature) and the new diagnostic correctly names the fixture + rc=0 + the real downgrade-banner text; reverted, re-validated clean. 15/15 clean on `/bin/bash` 3.2.57, 15/15 clean on `/opt/homebrew/bin/bash` 5.3.15 (both well above the 10-run bar), post-mutation-test re-validation also clean on both. The underlying attic script's own leak is NOT fixed here (out of scope; filed as backlog RETRY-GUARD-STATE-LEAK-DCG-01 + spawned task_8d243391) — this fix hardens the CALLER so the sweep itself is immune regardless.
- 2026-07-29: `docs/portability-baseline.txt` — M6 HOUSEKEEPING. Re-measured (not trusted from the verifier's numbers): discovered=170 pass=124 fail=46 (2026-07-29), header refreshed. Deleted 8 of the 9 rows the sweep's own STALE warning flagged as now-passing (cross-repo-nl-touch-warn.sh, lib/perf-ledger.sh, lib/session-heartbeat-lib.sh, scope-enforcement-gate.sh, agent-heartbeat.sh, needs-you.sh, nl-issue.sh, session-resumer.sh); deliberately KEPT `scripts/coord-sync.sh` despite it passing this run — its existing hand-added comment documents it as LOAD-SENSITIVE (passed 4/6, failed 2/6 across prior sweeps) with an explicit removal condition ("together with the fix that makes the suite load-independent") that a single passing measurement does not satisfy; removing it now would risk reintroducing the false-RED it was added to prevent. Two NEW unbaselined failures surfaced by this run (`scripts/loe-backfill.sh`, `scripts/manifest-check.sh`) were NOT added — they belong to other in-flight sessions' scripts (loe-backfill.sh's self-test gap is already tracked in docs/backlog.md by accountable-estate-program-2026-07 T7) and growing the baseline for them is outside this task's scope; `harness-doctor.sh --portability` may RED on these two for a cause that is not this suite.
