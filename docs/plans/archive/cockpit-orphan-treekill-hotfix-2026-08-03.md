...
- [x] 1. Add `killTree()` to `derive-cache.js` and call it from
...
- [x] 2. `server.js` `runAskRegistryCli`: kill the child tree on timeout
...
_Generated by close-plan.sh on 2026-08-03T16:42:02Z._

### 1. Implementation Summary

Plan: `docs/plans/cockpit-orphan-treekill-hotfix-2026-08-03.md` (slug: `cockpit-orphan-treekill-hotfix-2026-08-03`).

Files touched (per plan's `## Files to Modify/Create`):

- `docs/handoffs/2026-08-03-cockpit-nl-recursion-rootcause.md`
- `docs/plans/cockpit-orphan-treekill-hotfix-2026-08-03.md`
- `neural-lace/workstreams-ui/server/derive-cache.js`
- `neural-lace/workstreams-ui/server/server.js`

Commits referencing these files:

```
02ff2f32 fix(cockpit): stop tripping nl.sh's spawn breaker — two-lane serial refresh + bounded ceiling for cockpit children
0808f2d9 fix(workstreams-ui): subprocess-storm — push fast path replaces per-poll nl.sh spawns
08108f93 feat(observability O.4): Workstreams UI cockpit rebuild — thin view over derived truth
171bf281 fix(cockpit): survive minimal-env spawn parents — absolute bash + login shell + lobotomy self-report/restart
295d7038 fix(ask-workstreams): Task 15 — add-path oracle mis-classification (Stage 3c)
29b40480 r2: stale-tab auto-reload - /api/health ui_build_ms + client reload-on-change
35f8ee4d fix(cockpit-roadmap-redesign Round 16): operator walkthrough feedback — spacing, markdown rendering, kill reorder buttons, drag-and-drop, green running
362f4715 splice(cockpit-roadmap-redesign t4): server.js mount + index.html script tag (exact anchors per fragment; composed suites 23/242/173/45 green)
3ff03d20 fix(workstreams-ui): surface orchestrator-prime asks so the GUI is a live frame of reference
40414d02 splice(cockpit-roadmap-redesign t5): server.js mount + index.html script tag (fragments applied; suites 168/25 green)
411aa5a9 feat(ask-workstreams): Task 12 — background auditor + drift badges
41528381 feat(ask-workstreams): Task 15 — Backlog pane (render/add/dispose)
43be5c85 refactor(workstreams): rename neural-lace/conversation-tree-ui → neural-lace/workstreams-ui
43f76c28 sweep: nl-issues batch 2 — end-manifest false-block, cockpit FM-037 engine, denylist narrowing, lint+digest+doctrine (#95)
4c39a8ee feat(workstreams): foundation for cross-machine coordination + no-cache GUI fix
536e813d feat(workstreams-ui): operator-authoring substrate + My-tasks surface (Tasks 1/2/6)
58db4d9d wip(workstreams-ui): ORPHANED partial Tasks 7/8 work — builder died mid-session; UNVERIFIED, review before trusting
5bc0594f fix(workstreams-ui): timeout-kill the whole child TREE — stops the orphan-manufacture storm
668b8307 fix(cockpit): sweep the dead-file://-href CLASS — asks.js + backlog.js (COCKPIT-DEAD-FILE-HREF-RESIDUAL-01)
83179b39 feat(ask-workstreams): Task 11 — server read surface (/api/asks, /api/ask/<id>, lifecycle write) + payload-schema allowlist
8ee5a5a5 fix(cockpit Round 17 deliverables 2+2a): shared fenced-command renderer + fix dead Inbox file:// links
a82ebf31 build(cockpit-v2 task2): the per-machine export CLI + A4 derive-lib refactor
a96f5dd5 feat(ask-workstreams): Task 1 walking skeleton — mechanism-emitted progress log, end to end
b23905cd feat(ask-workstreams): Task 14 — My To-Do sidebar pane
b3ba920d build(cockpit-v2 task4): peer view in the cockpit (local coord-clone read)
bac3083d fix(wave-o O.4): cockpit acceptance-FAIL remediation (O.4-fix1)
c4db22b4 test(workstreams-ui): Tasks 7/8 e2e locks (T17-T20) + boot-safe zod require
ccdd03dd feat(cockpit-v2 task 1): unify plan-markdown parser + resolver into plan-parse.js
cf912683 build(cockpit-roadmap-redesign t3-int): wire task-1 derive-lib into roadmap-routes.js
d30413e4 fix(cockpit): to-do anti-noise respec — per-item noise_flag, never whole-payload 500 (operator-visible outage)
```
...
_Written by close-plan.sh at closure (2026-08-03T16:41:59Z)._

Outcome metric: no outcome metric declared by the plan at close time
Re-check date: 2026-08-17T09:41:59Z (default)
