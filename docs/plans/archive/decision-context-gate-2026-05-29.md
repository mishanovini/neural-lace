...
- 2026-05-30: `docs/decisions/047-decision-context-enforcement-surface.md` — ADR ID renumbered from 037 to 045 (037-044 already taken at dispatch time), then to 047 during the 2026-06 PT↔personal fork reconcile (045/046 held by the two workstreams ADRs on PT). Plan references to "037" in `## Files to Modify/Create` and `## Definition of Done` resolve to file 045. Same content, same Task 1, mechanical-only change.
- 2026-05-30: `docs/DECISIONS.md` — index row added for ADR 047 (later 047 — see fork-reconcile note above) (atomic with the new ADR per `decisions-index-gate.sh`).
...
- 2026-05-31 (Task 8): `docs/plans/decision-context-gate-2026-05-29-evidence-task8.md` — builder-authored Task 8 evidence + rung-2 comprehension articulation (matches per-task evidence file convention from Wave 2).
- 2026-05-31 (B5-FU-1 cross-task contract fix): `docs/plans/decision-context-gate-2026-05-29-evidence-b5fu1.md` — builder-authored rung-2 comprehension articulation for the B5-FU-1 stamp fix (matches per-task evidence file convention). The fix itself is to `adapters/claude-code/hooks/decision-context-gate.sh` (already in `## Files to Modify/Create`): inject `surfaced_by: "decision-context-gate"` into the shared `details` object at the emit-construction site (line 495) so both the primary event and the sibling `item-details-set` carry it. Task 5 pending-surfacer hard-requires this exact value (`details.surfaced_by === "decision-context-gate"`) — pre-fix the surfacer would surface NOTHING in production. ST28 self-test added (29/29 PASS); WS-1 round-trip confirmed carrying the stamp.
- 2026-05-31 (Task 9-full Wave 5): `neural-lace/conversation-tree-ui/web/app.css` — minimal additive CSS for the fence-grammar fields rendered by the `renderItemDetails` extension absorbed earlier in Wave 5 (`det-chip-*` urgency / reversibility / state colour-coding; `det-rec-*` recommendation block; `det-reply-with-box` monospace; `det-opt-field` per-option new-field rows). Pure-additive — zero overrides of existing selectors. The sibling `web/app.js` entry (already absorbed earlier in Wave 5) renders the chips with these class names; without app.css they would render as plain text (graceful degradation). No new build dependency; existing app.css is the canonical stylesheet.
- 2026-05-31 (Task 9-full Wave 5): `docs/plans/decision-context-gate-2026-05-29-evidence-task9-full.md` — builder-authored Task 9-full evidence + rung-2 comprehension articulation (matches per-task evidence file convention from Wave 2 and the sibling Task 8 / B5-FU-1 entries above).
