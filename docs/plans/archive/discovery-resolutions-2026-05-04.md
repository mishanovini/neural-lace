...
- 2026-05-04: `docs/plans/discovery-resolutions-2026-05-04-evidence.md` — task-verifier evidence-block companion file written by the verifier agent for tasks 1/2/3 per the evidence-first protocol. Sibling-of-plan convention; surfaced via in-flight update because the file is generated by task-verifier rather than authored by the planner.
- 2026-05-04: Task 1 acceptance criteria reinterpreted post-audit. The user's other session ran a stale-plan audit (commit `b825c2d`) on the 3 plans Task 1 archived and found 2 of 3 were "FALSELY COMPLETED" — `harness-quick-wins-2026-04-22.md` and `public-release-hardening.md` were un-archived (their `Status: COMPLETED` was inaccurate). Only `document-freshness-system.md` stays archived. This does NOT invalidate Task 1: the sweep mechanism correctly acted on what the files declared (`Status: COMPLETED`), and the user's audit was the right secondary check that caught the false-positives. Task 1's verification PASS reflects the sweep's correct mechanical behavior; HARNESS-GAP-15 (added by `b825c2d`) tracks the audit's followup work.
- 2026-05-04: `docs/harness-architecture.md` regression fix. Commit `2a49b11` (Task 4) inadvertently overwrote 8 rows that documented Phase 1d-C-2/1d-C-3 mechanisms — root cause was the very template-vs-live divergence Discovery #2 surfaced (live `~/.claude/docs/harness-architecture.md` was missing entries the user's other session had added to the repo). Caught by task-verifier on Task 4. Followup commit restores all 8 rows from canonical state at `e95313b` and re-applies the Task 4 additions on top.
