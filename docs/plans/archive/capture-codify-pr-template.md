...
- **Task A.1 (template):** verify the file exists at the correct path, contains all four required `##` sections (Summary, What changed and why, What mechanism would have caught this?, Testing performed), the three `### a/b/c)` answer-form sub-headings under the mechanism section, and the obviously-not-real placeholder text. Grep: `grep -c '^## ' .github/PULL_REQUEST_TEMPLATE.md` returns 4; `grep -c '^### ' .github/PULL_REQUEST_TEMPLATE.md` returns 3; `grep -c 'mechanism answer — replace this bracketed text' .github/PULL_REQUEST_TEMPLATE.md` returns 3 (one placeholder per answer-form sub-heading).
- **Task A.2 (workflow):** trigger the workflow via a throwaway PR (see Task A.7). Capture the full GitHub Actions run log showing fail-on-empty and pass-on-filled. Evidence must include the run URL and exit code. Workflow YAML must include `actions/checkout@v4` as the first job step (verified via `grep checkout@v4 .github/workflows/pr-template-check.yml`). Sole verification path is the real GitHub Actions run produced by Task A.7; Testing Strategy does not depend on optional local tooling.
...
- **Task A.14 (Testing Strategy cleanup):** verify the `act` tool reference no longer appears in the Task A.2 Testing Strategy bullet via `grep -A1 '\*\*Task A\.2 ' docs/plans/capture-codify-pr-template.md | grep -cE '\bact\b|\`act\`|act per Section'` returning 0 (matches the standalone tool name, not the substring inside `actions/checkout`). Section 4's optional-tooling mention of `act` is unaffected and remains.
