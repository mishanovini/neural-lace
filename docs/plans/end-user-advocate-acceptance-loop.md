...
- New hook `adapters/claude-code/hooks/product-acceptance-gate.sh` — Stop hook chained after `pre-stop-verifier.sh`, blocking session termination when an ACTIVE plan has no PASS artifact for the current session.
...
- [ ] 1.1. Create stub plan `docs/plans/acceptance-loop-smoke-test.md` with ONE acceptance scenario ("the user can navigate to a given URL and see expected text on the page"). Plan targets an existing stable page in the repo (e.g., the `README`-backed content page) so the smoke test has no code dependency.
...
- The walking skeleton can run against any deployable user-facing page in an existing project repo. If none is available, the smoke test uses a minimal Next.js stub that serves fixed content.
...
- This meta-plan is bootstrap-excluded from end-user-advocate review at plan-time because the agent doesn't exist yet. Subsequent plans (including the Phase 1 smoke-test plan, and the Phase 7 self-test) WILL undergo plan-time advocate review once Phase 1 ships.
